...
//20130821, TimLiu add for support ZyXEL customized firmware upgrade mechanism
...
//20130821, TimLiu add for support ZyXEL customized firmware upgrade mechanism
#if defined(ZYXEL) && defined(ZYXEL_RSA_FORMAT)
#define ZyTAG_LEN       2304		//256 + 1024*2
#endif
#define TAG_LEN         256
...
#define TOKEN_LEN       20
#if defined(ZYXEL) && defined(ZYXEL_RSA_FORMAT)
#define SDK_TOKEN_LEN   20
#define ZyTOKEN_LEN     1024
#define ZyEXTEND_TOKEN_LEN  (ZyTOKEN_LEN-TOKEN_LEN)
#endif
...
#if defined(ZYXEL) && defined(ZYXEL_RSA_FORMAT)
#define ZyRESERVED_LEN    (ZyTAG_LEN - TAG_VER_LEN - SIG_LEN - SIG_LEN_2 - CHIP_ID_LEN - BOARD_ID_LEN - \
                        (4*IMAGE_LEN) - (3*ADDRESS_LEN) - (3*FLAG_LEN) - (2*ZyTOKEN_LEN) - IMAGE_VER_LEN)

// TAG for downloadable image (kernel plus file system)
typedef struct _ZyFILE_TAG
{
    ...
    char reserved[ZyRESERVED_LEN];        // reserved for later use
    char imageValidationToken[ZyTOKEN_LEN];// image validation token - can be crc, md5, sha;  for
                                                 // now will be 4 unsigned char crc
    char tagValidationToken[ZyTOKEN_LEN]; // validation token for tag(from signiture_1 to end of // mageValidationToken)
} ZyFILE_TAG, *PZyFILE_TAG;
#endif
...
//20131031, TimLiu add for support ZyXEL customized firmware upgrade mechanism
#if defined(ZYXEL) && defined(ZYXEL_RSA_FORMAT)
#define PRODUCTNAME_LEN		NVRAM_PRODUCTNAME_LEN
#define FIRMWAREVERSION_LEN	NVRAM_FIRMWAREVERSION_LEN
#define FEATUREBITS_LEN		NVRAM_FEATUREBITS_LEN
#define BUILDINFO_LEN		NVRAM_BUILDINFO_LEN
#define WFI_ZYXEL_IMAGE_CHECKSUM_LEN	4
#define WFI_ZYXEL_HEADER_CHECKSUM_LEN	2
#define WFI_ZYXEL_RESERVED_LEN	\
	(ZyTOKEN_LEN - 20 - PRODUCTNAME_LEN - FIRMWAREVERSION_LEN - FEATUREBITS_LEN - BUILDINFO_LEN - WFI_ZYXEL_IMAGE_CHECKSUM_LEN*2 - WFI_ZYXEL_HEADER_CHECKSUM_LEN)
#endif
...
typedef struct _ZyWFI_TAG
{
    ...
    unsigned int wfiFlashType;
    unsigned int wfiFlags;
    char wfiProductName[PRODUCTNAME_LEN];
    char wfiFirmwareVersion[FIRMWAREVERSION_LEN];
    char FeatureBits[FEATUREBITS_LEN];
    char BuildInfo[BUILDINFO_LEN];
    unsigned int wfiKernelChecksum;
    unsigned int wfiRootFSChecksum;
    char wfichUnused[WFI_ZYXEL_RESERVED_LEN];
    unsigned short zy_checksum;
} __attribute__ ((aligned(2),packed)) ZyWFI_TAG, *ZyPWFI_TAG;
#endif
