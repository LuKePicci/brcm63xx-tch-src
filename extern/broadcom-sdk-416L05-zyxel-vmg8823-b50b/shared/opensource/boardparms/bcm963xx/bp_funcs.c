...
#if ((defined(_BCM963138_) || defined(CONFIG_BCM963138) || defined (_BCM963148_) || defined(CONFIG_BCM963148)) && defined(ZYXEL_SUPPORT_ETHWAN_AS_LAN) ) || (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD)) || (defined(ZYXEL_BOARD_DETECT_BY_GPIO))
#include "bcm_hwdefs.h"
#endif

#if (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD))
 /*!
 *  Special case VMG3925-B10B & VMG3925-B10C co-bootloader workaround.
 */
#include "63268_map_part.h"
/*!
 *  Get chip id and chip reversion from address.
 *
 *  @param[out] *chipIdInfo     chip id information.
 */
void zyGetChipIdInfo(char *chipIdInfo);
#endif
...
//#if defined(ZYXEL_WPS_SPOTLIGHT_2GPIO)
  bp_usGpioWPSSpotlightRed,
//#endif
  bp_usGpioWPSSpotlight,
//#endif
#endif
};
...
//20130821, TimLiu add for ZyXEL usage.
#if defined(ZYXEL) && (ZYXEL==1)
 MAP( bp_usGpioVoip1MessageLed, Voip1MsgLed),
 MAP( bp_usGpioVoip2MessageLed, Voip2MsgLed),
 MAP( bp_usGpioLedVdsl, LedVdsl),
 MAP( bp_usGpioLedVdslFail, VdslFail),
 MAP( bp_usGpioSecLedVdsl, SecVedAdsl),
 MAP( bp_usGpioSecLedVdslFail, SecLedVdslFail),
 MAP( bp_usGpioLedInternetData, IntLedData),
 MAP( bp_usGpioLedInternetFail, IntLedFail),
 MAP( bp_usGpioLedUSB0, Usb0Led),
 MAP( bp_usGpioLedUSB0Fail, Usb0LedFail),
 MAP( bp_usGpioLedUSB1, Usb1Led),
 MAP( bp_usGpioLedUSB1Fail, Usb1LedFail),
 MAP( bp_usGpioLedLTESignal1, LteLedSignal1),
 MAP( bp_usGpioLedLTESignal2, LteLedSignal2),
 MAP( bp_usGpioLedLTESignal3, LteLedSignal3),
 MAP( bp_usGpioLedLTESignal4, LteLedSignal4),
 MAP( bp_usGpioLedSesWireless2, WPSWireless2),
#ifdef ZYPRJ_VMG8948
 MAP( bp_usGpioLedGuestWiFi, GuestWifiLed),
 MAP( bp_usGpioLedGuestWPS, GuestWpsLed),
#endif
//#if defined(ZYXEL_WPS_SPOTLIGHT) 
//#if defined(ZYXEL_WPS_SPOTLIGHT_2GPIO)
 MAP( bp_usGpioWPSSpotlightRed, WPSSpotlightRedLED),
//#endif
 MAP( bp_usGpioWPSSpotlight, WPSSpotlightLED),
//#endif
#endif
 {-1, NULL}
};
...
//20130821, TimLiu add for ZyXEL usage.
  ...
  bp_usGpio5GReset,	//using for reset 5g Quantenna module
  bp_usGpioSelectWanLed,	//XMG3512 using the GPIO to secelt output WAN signal to WAN LED or SFP LED
  // Using GPIO to simulate MDIO/MDC for external switch, RTL8367RB-VB on VMG3925, DSL-491GNAU-B1BC.
  bp_usGpioMDIO_SDA,
  bp_usGpioMDC_SCK,
  bp_usGpioLedLanAggregateLink, //ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED related
  bp_usGpioLedLanAggregateAct,  //ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED related
  bp_usGpio24GReset,
  bp_usGpio5GDisable,
  bp_usGpio24GDisable,
  bp_usGpioBoardDetect0,    //ZYXEL_BOARD_DETECT_BY_GPIO related
  bp_usGpioBoardDetect1,
  bp_usGpioBoardDetect2,
  bp_usGpioBoardDetect3,
  bp_usGpioBoardDetect4,
  bp_usGpioZigbeeReset,     //Reset Zigbee module.
  bp_usGpioZigbeeStopBtl,   //Let Zigbee stop in bootloader CLI, not enter app.
  bp_usGpioZigbeePwrEn,      //Supply power for Zigbee module.
  bp_usGpioSelectWanLedEth  //ex. VMG8825-B50B_Generic: software control WAN LED
...
#if ((defined(_BCM963138_) || defined(CONFIG_BCM963138) || defined (_BCM963148_) || defined(CONFIG_BCM963148)) && defined(ZYXEL_SUPPORT_ETHWAN_AS_LAN)) || (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD))
/**************************************************************************
* Name       : bpstrlen
*
...
***************************************************************************/
static size_t  bpstrlen(const char *str)
{
	const char *s;
...
*
***************************************************************************/
static void bpmemcpy(void *dest, const void *source, size_t num) {
    int i = 0;
    // casting pointers
...
***************************************************************************/
static void *bpmemset(void *s, int c, size_t n)
{
    unsigned char* p=s;
    while(n--)
        *p++ = (unsigned char)c;
...
int BpSetBoardId( char *pszBoardId )
{
	...
	char runtime_BoardId[NVRAM_BOARD_ID_STRING_LEN];
	char tmp_pszBoardId[NVRAM_BOARD_ID_STRING_LEN];
#elif (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD))
    char tmp_pszBoardId[NVRAM_BOARD_ID_STRING_LEN];
    char chipIdInfo[BRCM_MAX_CHIP_NAME_LEN];
#endif

#if (defined(_BCM963138_) || defined(CONFIG_BCM963138) || defined (_BCM963148_) || defined(CONFIG_BCM963148)) && defined(ZYXEL_SUPPORT_ETHWAN_AS_LAN)
	bpmemset(runtime_BoardId, 0x0, NVRAM_BOARD_ID_STRING_LEN);
	bpmemset(tmp_pszBoardId, 0x0, NVRAM_BOARD_ID_STRING_LEN);
	if(isEthwanAsWan)
		bpmemcpy(runtime_BoardId, RUNTIME_BOARDID_WAN, bpstrlen(RUNTIME_BOARDID_WAN));
	else
		bpmemcpy(runtime_BoardId, RUNTIME_BOARDID_LAN, bpstrlen(RUNTIME_BOARDID_LAN));
	printk("BpSetBoardId:Current Board ID was %s\n",pszBoardId);
	if(!bpstrcmp(pszBoardId, RUNTIME_BOARDID_WAN) || !bpstrcmp(pszBoardId, RUNTIME_BOARDID_LAN) )
	{
		// Don`t modify the original "pszBoardId".
		// In kernel side, it will let the checksum of inMemNvramData become different.
		bpmemcpy(tmp_pszBoardId, runtime_BoardId, bpstrlen(runtime_BoardId));
		printk("BpSetBoardId:Runtime board ID will be %s\n", tmp_pszBoardId );
	}else
		bpmemcpy(tmp_pszBoardId, pszBoardId, bpstrlen(pszBoardId));
#elif (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD))
    // Special case VMG3925-B10B & VMG3925-B10C co-bootloader workaround.
    bpmemset(tmp_pszBoardId, 0x0, NVRAM_BOARD_ID_STRING_LEN);
    bpmemset(chipIdInfo, 0x0, BRCM_MAX_CHIP_NAME_LEN);

    zyGetChipIdInfo(chipIdInfo);

    if (!bpstrcmp(pszBoardId, RUNTIME_BOARDID_VMG3925_B10B) &&
        !bpstrcmp(chipIdInfo, CHIPID_63167))
    {
        bpmemcpy(tmp_pszBoardId, RUNTIME_BOARDID_VMG3925_B10C, bpstrlen(RUNTIME_BOARDID_VMG3925_B10C));
    }
    else
    {
        bpmemcpy(tmp_pszBoardId, pszBoardId, bpstrlen(pszBoardId));
...
#if ((defined(_BCM963138_) || defined(CONFIG_BCM963138) || defined (_BCM963148_) || defined(CONFIG_BCM963148)) && defined(ZYXEL_SUPPORT_ETHWAN_AS_LAN)) || (defined(ZYPRJ_VMG3926) && defined(SAME_FW_FOR_TWO_BOARD))
        if( !bpstrcmp((*ppcBp)[0].u.cp, tmp_pszBoardId) ) {
#else
...
int BpGetAddlWanPort(unsigned char *pPort)
{
    const ETHERNET_MAC_INFO* pEnetInfosArray = BpGetEthernetMacInfoArrayPtr();
    if (pEnetInfosArray != NULL)
    {
        unsigned char unit = 1, port;
        unsigned long port_map, port_flags;
        const ETHERNET_MAC_INFO *info = &pEnetInfosArray[unit];
        if (info == NULL)
        {
            printk("\n\nERROR : BpGetExpAddlWanPort - info is NULL\n\n\n");
            return BP_VALUE_NOT_DEFINED;
        }
        port_map = info->sw.port_map; /* Get port map for external switch */

        for (port = 0; port_map && (port < BP_MAX_SWITCH_PORTS); port++) /* go through all the switch ports */
        {
...
int BpGetSensorEventExtIntr( unsigned short *pusValue )
{
    return( BpGetUs(bp_usExtIntrSensorEvent, pusValue ) );
} /* BpGetSensorEventExtIntr */
...
extern int board_zyxelFeatureBitsWifi24GFemGet(void);
#endif
int BpUpdateWirelessSromMap(unsigned short chipID, unsigned short* pBase, int sizeInWords)
{
...
#if !defined(ZYXEL_FEATURE_BITS) && defined(ZYXEL_FEM_DETECT)
...
#if defined(ZYXEL_FEATURE_BITS)
    if (board_zyxelFeatureBitsWifi24GFemGet())
    {
        goto patch_extra_srom;
    }
#elif defined(ZYXEL_FEM_DETECT)
	if( BpGetDetectFEMGpio(&bpGpio) == BP_SUCCESS )
	{
		...
		printk("wl: FEM select GPIO_%d=%d\n",(bpGpio&BP_GPIO_NUM_MASK),gpioValue);
		if( (gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh) )
			goto patch_extra_srom;
	}
    else
    {
        // Co-fw for VMG3925-B10C(FEM GPIO exist) and VMG3925-B10B(FEM GPIO not exist).
        printk("wl: FEM GPIO not exist.\n");
    }
#endif
    ...
    while ( wlanPaInfo[i].szboardId[0] != 0 ) {
        ...
        if ( !bpstrcmp(g_pCurrentBp[0].u.cp, wlanPaInfo[i].szboardId) ) {
            ...
            if ( (wlanPaInfo[i].usWirelessChipId == chipID) && (wlanPaInfo[i].usNeededSize <= sizeInWords) ){
                ...
                while ( !(wlanPaInfo[i].uniqueEntries[j].wordOffset == 0 && wlanPaInfo[i].uniqueEntries[j].value == 0) ) {
...
patch_extra_srom:
    i = 0;
    while ( wlanPaInfoExtra[i].szboardId[0] != 0 ) {
        /* check boardId */
        if ( !bpstrcmp(g_pCurrentBp[0].u.cp, wlanPaInfoExtra[i].szboardId) ) {
            /* check chipId */
            if ( (wlanPaInfoExtra[i].usWirelessChipId == chipID) && (wlanPaInfoExtra[i].usNeededSize <= sizeInWords) ){

			/* valid , patch common to multiple boards entry */
                while ( wlanPaInfoExtra[i].commonEntries[j].wordOffset != 0) {
                    pBase[wlanPaInfoExtra[i].commonEntries[j].wordOffset] = wlanPaInfoExtra[i].commonEntries[j].value;
                    j++;
                }

                j = 0;
                /* valid , patch board specific entry */
                while ( !(wlanPaInfoExtra[i].uniqueEntries[j].wordOffset == 0 && wlanPaInfoExtra[i].uniqueEntries[j].value == 0) ) {
                    pBase[wlanPaInfoExtra[i].uniqueEntries[j].wordOffset] = wlanPaInfoExtra[i].uniqueEntries[j].value;
                    j++;
                }
...
/*ZyXEL Porting , yushiuan add start , originally the function is defined in boardparm.c ,2015/01/12*/
//Michael.20130618.001.B: Add
#if defined(ZYXEL) && (ZYXEL==1)
int BpGetPhone1MWILedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioVoip1MessageLed, pusValue ) );
} /* BpGetPhone1MWILedGpio */


int BpGetPhone2MWILedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioVoip2MessageLed, pusValue ) );
} /* BpGetPhone2MWILedGpio */
#endif //defined(ZYXEL) && (ZYXEL==1).
//Michael.20130618.001.E: Add
/*ZyXEL Porting , yushiuan add end , originally the function is defined in boardparm.c*/
...
/**************************************************************************
* Name       :  BpGetLanAggregateLinkGpio
*               BpGetLanAggregateActGpio
* Description:
...
*              BP_VALUE_NOT_DEFINED - At least one return value is not defined
*                  for the board.
* Note       : ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED related
***************************************************************************/
int BpGetLanAggregateLinkGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLanAggregateLink, pusValue ) );
}
int BpGetLanAggregateActGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLanAggregateAct, pusValue ) );
}
...
int BpGetExtNtrPinMux( void ) {
    ...
    for( pElem = g_pCurrentBp; pElem && (pElem->id != bp_last); pElem++ ) {
        /* Check if board has NTR reference input */
        if( pElem->id == bp_usNtrRefIn ) {
            /* Look for the pin that board uses */
            for (i = 0 ; g_pinmux_defs[i].id  != bp_last ; i++) {
                if ((g_pinmux_defs[i].id == bp_usNtrRefIn) && (g_pinmux_defs[i].port == -1) ) {
                    if ((g_pinmux_defs[i].func == (pElem->u.us & BP_GPIO_NUM_MASK))
                        || (g_pinmux_defs[i].func == -1)) {
                        return g_pinmux_defs[i].mux_info;
                    }
                }
            }
        }
...
///20130821, TimLiu add for access ZyXEL board parameters
#if defined(ZYXEL) && (ZYXEL==1)
#if defined(ZYPRJ_VMG8948)
/**************************************************************************
* Name       : BpGetGuestWirelessLedGpio
*
* Description: This function returns the GPIO pin assignment for the Guest
*              Wireless Led.
*
* Parameters : [OUT] pusValue - Address of short word that the Guest Wireless
*                  Led GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*                  for the board.
***************************************************************************/
int BpGetGuestWirelessLedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedGuestWiFi, pusValue ) );
} /* BpGetGuestWirelessLedGpio */

/**************************************************************************
* Name       : BpGetGuestWPSLedGpio
*
* Description: This function returns the GPIO pin assignment for the Guest
*              Wireless WPS Led.
*
* Parameters : [OUT] pusValue - Address of short word that the Guest Wireless
*                  WPS Led GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
*              BP_VALUE_NOT_DEFINED - At least one return value is not defined
*                  for the board.
***************************************************************************/
int BpGetGuestWPSLedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedGuestWPS, pusValue ) );
} /* BpGetGuestWPSLedGpio */

/**************************************************************************
* Name       : BpGetGuestWirelessExtIntr
*
* Description: This function returns the external interrupt number for the
*              Wireless Ses Button.
*
...
*                  for the board.
***************************************************************************/
int BpGetGuestWirelessExtIntr( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioGuestWirelessBtn, pusValue ) );
} /* BpGetGuestWirelessExtIntr */

/**************************************************************************
* Name       : BpGetGuestWirelessExtIntrGpio
*
* Description: This function returns the external interrupt number for the
*              Guest Wireless Button.
*
* Parameters : [OUT] pusValue - Address of short word that the Guest Wireless
*                  external interrup is returned in.
*
...
*                  for the board.
***************************************************************************/
int BpGetGuestWirelessExtIntrGpio( unsigned short *pusValue )
{
    ...
    pelem = BpGetElem(bp_usGpioGuestWirelessBtn,  &start_elem, bp_last);
    if (bp_usGpioGuestWirelessBtn != pelem->id)
    {
       *pusValue = BP_NOT_DEFINED;
        ret      = BP_VALUE_NOT_DEFINED;
    }
    ...
    return( ret );
} /* BpGetGuestWirelessExtIntrGpio */
#if defined(ZYXEL_WIFI_LED_PATTERN) && (ZYXEL_WIFI_LED_PATTERN ==1)
int BpGetWirelessSelectGpio( unsigned short *pusValue )
{
	return( BpGetUs(bp_usGpioWirelessSelect, pusValue ) );
}
#endif //defined(ZYXEL_WIFI_LED_PATTERN) && (ZYXEL_WIFI_LED_PATTERN ==1).
#endif //defined(ZYPRJ_VMG8948).

// to provide power for Internet Green LED on VMG9823 and VMG3925(DSL-491GNAU-B1BC).
int BpGetInternetLedEnableGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioInternetLedEnable, pusValue ) );
}

// to select which signal will output to Internet LED on VMG9823 and VMG3925(DSL-491GNAU-B1BC).
int BpGetInternetCtrlSignalGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioInternetCtrlSignal, pusValue ) );
}

// Using GPIO to simulate MDIO/MDC for external switch, RTL8367RB-VB on VMG3925, DSL-491GNAU-B1BC.
int BpGetMDIO_SDAGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioMDIO_SDA, pusValue ) );
}

// Using GPIO to simulate MDIO/MDC for external switch, RTL8367RB-VB on VMG3925, DSL-491GNAU-B1BC.
int BpGetMDC_SCKGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioMDC_SCK, pusValue ) );
}

// Interrupt from RTL8367RB-VB on VMG3925, DSL-491GNAU-B1BC.
int BpGetExtSwitchInitExtIntr( unsigned short *pusValue )
{
    return( BpGetUs(bp_usExtIntrExtSwitchInit, pusValue ) );
}

int BpGetDECTResetGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioDECTReset, pusValue ) );
} /* BpGetWirelessSesExtIntr */

int BpGetDECTExtIntr( unsigned short *pusValue )
{
    return( BpGetUs(bp_usExtIntrDECT, pusValue ) );
} /* BpGetWirelessSesExtIntr */

int BpGetDECTExtIntrGpio( unsigned short *pusValue )
{
    bp_elem_t *pelem;
    int ret;
    bp_elem_t *start_elem = 0;
    ...
    /* First go to elem with id bp_usExtIntrDECT */
    pelem = BpGetElem(bp_usExtIntrDECT, &start_elem, bp_last);
    if (bp_usExtIntrDECT != pelem->id)
    {
       *pusValue = BP_NOT_DEFINED;
        ret      = BP_VALUE_NOT_DEFINED;
    }
...
*                  for the board.
***************************************************************************/
int BpGetWirelessSesLed2Gpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedSesWireless2, pusValue ) );
} /* BpGetWirelessSes2LedGpio */

/**************************************************************************
* Name       : BpGetVoip1MessageGpio
*
* Description: This function returns the GPIO pin assignment for the voip1 message
*              LED.
*
...
*                  for the board.
***************************************************************************/
int BpGetVoip1MessageGpio( unsigned short *pusValue )
{
	//return( BpGetUs(bp_usGpioVoip1MessageLed, pusValue ) );
	return( BpGetGpio(bp_usGpioVoip1MessageLed, pusValue ) );
}

/**************************************************************************
* Name       : BpGetVoip2MessageGpio
*
* Description: This function returns the GPIO pin assignment for the voip2 message
*              LED.
*
* Parameters : [OUT] pusValue - Address of short word that the VDSL LED
*                  GPIO pin is returned in.
...
*                  for the board.
***************************************************************************/
int BpGetVoip2MessageGpio( unsigned short *pusValue )
{
	//return( BpGetUs(bp_usGpioVoip2MessageLed, pusValue ) );
	return( BpGetGpio(bp_usGpioVoip2MessageLed, pusValue ) );
}
...
*              LED that is used when there is a DSL connection failure.
*
* Parameters : [OUT] pusValue - Address of short word that the VDSL fail LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*
* Description: This function returns the GPIO pin assignment for the second VDSL
*              LED of the Secondary line, applicable more for bonding.
*
* Parameters : [OUT] pusValue - Address of short word that the second VDSL LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*              a DSL connection failure.
*
* Parameters : [OUT] pusValue - Address of short word that the second VDSL fail LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*
* Description: This function returns the GPIO pin assignment for the USB0
*              LED.
*
* Parameters : [OUT] pusValue - Address of short word that the USB0 LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*
* Description: This function returns the GPIO pin assignment for the USB0
*              LED that is used when there is a connection failure.
*
* Parameters : [OUT] pusValue - Address of short word that the USB0 fail LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*
* Description: This function returns the GPIO pin assignment for the USB1
*              LED.
*
* Parameters : [OUT] pusValue - Address of short word that the USB1 LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
*              LED that is used when there is a connection failure.
*
* Parameters : [OUT] pusValue - Address of short word that the USB1 fail LED
*                  GPIO pin is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
{
    return( BpGetUs(bp_usExtIntrWirelessBtn, pusValue ) );
} /* BpGetWirelessBtnGpio */

int BpGetWirelessBtnExtIntrGpio( unsigned short *pusValue )
{
    bp_elem_t *pelem;
    int ret;
    bp_elem_t *start_elem = 0;
    ...
    /* First go to elem with id bp_usExtIntrWirelessBtn */
    pelem = BpGetElem(bp_usExtIntrWirelessBtn, &start_elem, bp_last);
    if (bp_usExtIntrWirelessBtn != pelem->id)
    {
       *pusValue = BP_NOT_DEFINED;
        ret      = BP_VALUE_NOT_DEFINED;
    }
...
int BpGetSFPDetectGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioSFPDetect, pusValue ) );
}

int BpGetSFPTXFaultGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioSFPTXFault, pusValue ) );
}

int BpGetSFPTXDisableGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioSFPTXDisable, pusValue ) );
}

int BpGetHTPGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioHTP, pusValue ) );
}

int BpGetLTESignal1LedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLTESignal1, pusValue ) );
}

int BpGetLTESignal2LedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLTESignal2, pusValue ) );
}

int BpGetLTESignal3LedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLTESignal3, pusValue ) );
}

int BpGetLTESignal4LedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioLedLTESignal4, pusValue ) );
}
//#if defined(ZYXEL_WPS_SPOTLIGHT)
//#if defined(ZYXEL_WPS_SPOTLIGHT_2GPIO)
int BpGetWpsSpotlightRedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioWPSSpotlightRed, pusValue ) );
}
//#endif
int BpGetWPSSpotlightGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioWPSSpotlight, pusValue ) );
}
//#endif
int BpGetWlanDisableGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioWlanDisable, pusValue ) );
}

int BpGet24GDisableGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpio24GDisable, pusValue ) );
}

int BpGet5GDisableGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpio5GDisable, pusValue ) );
}

int BpGet24GResetGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpio24GReset, pusValue ) );
}

int BpGet5GResetGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpio5GReset, pusValue ) );
}

int BpGetSelectWanLedGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioSelectWanLed, pusValue ) );
}
int BpGetSelectWanLedGpioEth( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioSelectWanLedEth, pusValue ) );
}
//#if defined(ZYXEL_FEM_DETECT)
int BpGetDetectFEMGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioWirelessFEM, pusValue ) );
}
//#endif
#if defined(ZYXEL_BOARD_DETECT_BY_GPIO)
/*!
 *  Board Detect By GPIO.
 *
 *  @param[in] index        Index of bit.
 *  @param[out] pusValue    Board detected GPIO pin.
 *  @return ret             BP_SUCCESS:success(0),other:fail.
 */
int BpGetBoardDetectGpio( unsigned short *pusValue, int index )
{
    int ret = 0;
    ...
    {
        case 0:
            ret = BpGetGpio(bp_usGpioBoardDetect0, pusValue);
            break;
        case 1:
            ret = BpGetGpio(bp_usGpioBoardDetect1, pusValue);
            break;
        case 2:
            ret = BpGetGpio(bp_usGpioBoardDetect2, pusValue);
            break;
        case 3:
            ret = BpGetGpio(bp_usGpioBoardDetect3, pusValue);
            break;
        case 4:
            ret = BpGetGpio(bp_usGpioBoardDetect4, pusValue);
            break;
        default:
            ret = -1;
            break;
...
#endif
int BpGetZigbeeResetGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioZigbeeReset, pusValue ) );
}
int BpGetZigbeeStopBtlGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioZigbeeStopBtl, pusValue ) );
}
int BpGetZigbeePwrEnGpio( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioZigbeePwrEn, pusValue ) );
}
#endif //defined(ZYXEL) && (ZYXEL==1).
