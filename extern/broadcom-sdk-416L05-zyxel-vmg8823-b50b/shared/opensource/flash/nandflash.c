...
#define HYNIX_H27U2G8F2C        0xda   //20130821, TimLiu SBG3600 using this

/* Toshiba flash parts. */
#define TOSHIBA_TC58NVG0S3E		0xd1   //20130821, TimLiu VMG5313 using this
#define TOSHIBA_TH58NVG2S3H		0xdc   //20170929, VMG4927-B50A using this
#define TOSHIBA_TC58NVG1S3HTAI0 0xda   //20171128, VMG3927-B50B using this

/* MXIC flash parts */
#define MXIC_MX30LF1208AA       0xf0
#define MXIC_MX30LF1G18AC       0xf1    //2017, 2nd source for VMG8924-B10D,VMG3925-B10C,XMG3512-B10A
#define MXIC_MX30LF2G18AC       0xda    //Zyxel,VMG8825-B50B_WIND, 256MB
#define MXIC_MX30LF4G18AC       0xdc    //Zyxel,VMG8825-B50B_WIND, 512MB
...
#define NAND_FLASH_DEVICES                                                    \
  {{NAND_MAKE_ID(FLASHTYPE_SAMSUNG,SAMSUNG_K9F5608U0A),"Samsung K9F5608U0"},  \
   ...
   {NAND_MAKE_ID(FLASHTYPE_MXIC,MXIC_MX30LF1G18AC),"MXIC MX30LF1G18AC"},      \
   {NAND_MAKE_ID(FLASHTYPE_MXIC,MXIC_MX30LF2G18AC),"MXIC MX30LF2G18AC"},      \
   {NAND_MAKE_ID(FLASHTYPE_MXIC,MXIC_MX30LF4G18AC),"MXIC MX30LF4G18AC"},      \
   ...
   {NAND_MAKE_ID(FLASHTYPE_ESMT,ESMT_F59L1G81MA),"ESMT F59L1G81MA"},          \
   {NAND_MAKE_ID(FLASHTYPE_ZENTEL,ZENTEL_A5U1GA31ATS),"Zentel A5U1GA31ATS"},  \
   {NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TC58NVG0S3E),"TOSHIBA TC58NVG0S3E"},\
   {NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TH58NVG2S3H),"TOSHIBA TH58NVG2S3H"},\
   {NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TC58NVG1S3HTAI0),"TOSHIBA TC58NVG1S3HTAI0"},\
   {0,""}                                                                     \
  }
    ...
    int offset, int numbytes)
{
    ...
    else
    {
...
#endif
        {
            for( i = 0; i < numbytes; i += sizeof(long) )
            {
                *buf = *(unsigned long *) &cache[i+offset];
                buf++;
            }
        }
    }
...
{
    int ret = FLASH_API_ERROR;
#if defined(NAND_READBACK_CHECK)
	int writelen = len;
	unsigned char *checkbuf = NULL;
#endif
...
#if defined(NAND_READBACK_CHECK)
	checkbuf = KMALLOC(writelen, 0);
	if(checkbuf == NULL) {
		printf("Error[%s(%d)]: No memory to allocate\n", __FUNCTION__, __LINE__);
	}else{
		if(nandflash_read_page(pchip, page_addr, checkbuf, writelen) != FLASH_API_OK)
		{
			ret = FLASH_API_ERROR;
		}else{
			if( memcmp(checkbuf, buffer, writelen) != 0){
				printf("Error: ReadBack Check Fail, mark bad blcok at 0x%8.8lx\n", page_addr);
				nand_mark_bad_blk(pchip, page_addr & ~(pchip->chip_page_size - 1));
				ret = FLASH_API_ERROR;
			}else{
				ret = FLASH_API_OK;
...
//20130821, TimLiu add to support AT commands to do flash access.
#if defined(ZYXEL) && defined(ZYXEL_AT_COMMANDS)
/***************************************************************************
 * Function Name: nand_flash_sector_erase_with_jffs2_cleanmark
 * Description  : Erase the specfied flash sector and write JFFS2 cleanmark int page 0.
 * Returns      : FLASH_API_OK or FLASH_API_ERROR
 ***************************************************************************/
int nand_flash_sector_erase_with_jffs2_cleanmark(unsigned short);
int nand_flash_sector_erase_with_jffs2_cleanmark(unsigned short blk)
{
    int ret = FLASH_API_OK;
    PCFE_NAND_CHIP pchip = &g_chip;
...
//20151029 TINA MODIFIED
#if 0
/***************************************************************************
 * Function Name: nandflash_force_block_erase
 * Description  : Erases a block without checking if it is a bad block.
 * Returns      : FLASH_API_OK or FLASH_API_ERROR
 ***************************************************************************/
static int nandflash_force_block_erase(int blk )
{
    PCFE_NAND_CHIP pchip = &g_chip;
    int ret = FLASH_API_OK;
    //int blk = blk_addr/pchip->chip_block_size;
    unsigned long blk_addr = blk * pchip->chip_block_size;
    ...
    DBG_PRINTF(">> nandflash_force_block_erase - addr=0x%8.8lx, ret=%d\n", blk_addr,
        ret);

    return( ret );
} /* nandflash_force_block_erase */
#endif
/***************************************************************************
 * Function Name: nand_force_mark_bad_blk
 * Description  : Force Mark/unmark the specified block as bad by writing 0xFFs to the
 *                spare area and updating the in memory bad block table.
 * Returns      : None.
 ***************************************************************************/
void nand_force_mark_bad_blk(int blk, int mark);
void nand_force_mark_bad_blk(int blk, int mark)
{
    PCFE_NAND_CHIP pchip = &g_chip;
    static int marking_bad_blk = 0;
    ...
    {
        marking_bad_blk = 1;
        DBG_PRINTF(">> nand_force_mark_bad_blk - addr=0x%8.8lx, block=%d\n",
            page_addr, blk);

        memset(spare, 0xff, pchip->chip_spare_size);

        /* Read the current spare area. */
        nandflash_read_spare_area(pchip,page_addr,spare,pchip->chip_spare_size);
        ...
        //nandflash_force_block_erase(blk);//20151029 TINA MODIFIED
        flash_dev_specific_cmd(FORCE_ERASE, &blk, NULL);

#if (INC_BTRM_BOOT==1) && defined(_BCM963268_)
        if( page_addr / pchip->chip_block_size > 2 )
#endif
        {
            /* Mark the first page in the block as bad so the Linux NAND
             * flash driver finds it when updating the BBT.
             */
            ret = nandflash_write_spare_area(pchip, page_addr, spare,
                pchip->chip_spare_size);
            if( ret == FLASH_API_ERROR )
                DBG_PRINTF("\nError writing clean marker, page_addr=%x\n", page_addr);
...
} /* nand_force_mark_bad_blk */

void dump_flashInfo(void);
void dump_flashInfo(void)
{
	PCFE_NAND_CHIP pchip = &g_chip;
	printf("chip_name = %s\n", pchip->chip_name);
	printf("chip_total_size = %8.8lx\n", pchip->chip_base);
	printf("chip_total_size = %d\n", pchip->chip_total_size);
	printf("chip_block_size = %d\n", pchip->chip_block_size);
	printf("chip_page_size = %d\n", pchip->chip_page_size);
	printf("chip_spare_size = %d\n", pchip->chip_spare_size);
	printf("chip_spare_step_size = %d\n", pchip->chip_spare_step_size);
}

int dump_spare_by_pages(int blk_number, int pag_number);
int dump_spare_by_pages(int blk_number, int pag_number)
{
    PCFE_NAND_CHIP pchip = &g_chip;
    unsigned long spare[SPARE_MAX_SIZE / 4];
    unsigned long page_addr = blk_number * pchip->chip_block_size;
    int max_page_number = pchip->chip_block_size / pchip->chip_page_size;
    int max_block_number = pchip->chip_total_size / pchip->chip_block_size;
    int ret = 0;

    if( (blk_number >= max_block_number) || (pag_number >= max_page_number) )
    {
    	printf("Block number or page number is out of range!(%d,%d)",blk_number ,pag_number);
    	return -1;
    }

	if( (ret = nandflash_read_spare_area( pchip, page_addr +
		(pag_number * pchip->chip_page_size), (unsigned char *) spare,
		pchip->chip_spare_size)) == FLASH_API_OK )
	{
...
int dump_spare_pages(int blk)
{
    ...
    {
    	if(dump_spare_by_pages(blk, i) == FLASH_API_ERROR)
    		return -1;
    }
