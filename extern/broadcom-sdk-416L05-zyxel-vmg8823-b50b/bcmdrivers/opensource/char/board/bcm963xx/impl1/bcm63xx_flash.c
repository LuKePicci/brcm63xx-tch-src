 ...
 */
extern void zyGetChipIdInfo(char *chipIdInfo);
#endif
...
static unsigned char *inMemZyFwInfo_buf;
static ZY_FW_INFO inMemZyFwInfo;

/** get ZyFwInfo data
 *
 * since it reads from in-memory copy of the ZyFwInfo data, always successful.
 */
void kerSysZyFwInfoGet(char *string, int strLen, int offset)
{
    memcpy(string, inMemZyFwInfo_buf + offset, strLen);
    return;
}

/** load nvram data from mtd device
 */
void kerSysZyFwInfoLoad(void)
{
	struct file * fp;
	mm_segment_t fs;
	int seq = 0;//kerSysGetSequenceNumber(1);
	...
	strcpy(cferam_fmt, fname);
	strcat(cferam_fmt, "%3.3d");

	inMemZyFwInfo_buf = (unsigned char *) &inMemZyFwInfo;
	memset(inMemZyFwInfo_buf, 0x00, ZY_FW_INFO_SIZE);

	for(seq=0;seq<=999;seq++){
		sprintf(cferam_buf, cferam_fmt, seq);
		fp = filp_open(cferam_buf, O_RDONLY, 0);
		if (!IS_ERR(fp) && fp->f_op && fp->f_op->read)
		{
			fp->f_pos = CFERAM_HEADER_LENGTH + ZY_FW_INFO_OFFSET;

			if(ZY_FW_INFO_SIZE != fp->f_op->read(fp, (void *) inMemZyFwInfo_buf, ZY_FW_INFO_SIZE, &fp->f_pos)) {
				printk("%s: Failed to read ZyFwInfo from '%s'\n", __FUNCTION__, cferam_buf);
			}
...
void kerSysNvRamGetBoardIdLocked(char *boardId)
{
    ...
    memset(chipIdInfo, 0x0, BRCM_MAX_CHIP_NAME_LEN);
    zyGetChipIdInfo(chipIdInfo);

    if (!strncmp(inMemNvramData.szBoardId, RUNTIME_BOARDID_VMG3925_B10B, strlen(RUNTIME_BOARDID_VMG3925_B10B)) &&
        !strncmp(chipIdInfo, CHIPID_63167, strlen(CHIPID_63167)))
    {
        memcpy(boardId, RUNTIME_BOARDID_VMG3925_B10C, strlen(RUNTIME_BOARDID_VMG3925_B10C));
    }
    else
    {
#endif
...
/* backup the NVRAM on the target board to the new NVRAM on the upgrade image.*/
void backupNVRAM(PNVRAM_DATA srcData,PNVRAM_DATA destData)
{
	int i=0;
	/*backup the serial Number in MRD info*/
	if(strncmp(srcData->SerialNumber, destData->SerialNumber, NVRAM_SERIALNUMBER_LEN)!=0)
		memcpy(destData->SerialNumber,srcData->SerialNumber, NVRAM_SERIALNUMBER_LEN);
	/*backup the MAC address on the target board*/
	if(strncmp((char *)srcData->ucaBaseMacAddr, (char *)destData->ucaBaseMacAddr, NVRAM_MAC_ADDRESS_LEN)!=0)
		memcpy(destData->ucaBaseMacAddr,srcData->ucaBaseMacAddr, NVRAM_MAC_ADDRESS_LEN);
	if(srcData->EngDebugFlag != destData->EngDebugFlag)
		destData->EngDebugFlag = srcData->EngDebugFlag;

	/*backup the partition information*/
	for(i=0; i<NP_TOTAL; i++)
	{
		if(srcData->ulNandPartOfsKb[i] != destData->ulNandPartOfsKb[i])
			destData->ulNandPartOfsKb[i] = srcData->ulNandPartOfsKb[i];
		if(srcData->ulNandPartSizeKb[i] != destData->ulNandPartSizeKb[i])
			destData->ulNandPartSizeKb[i] = srcData->ulNandPartSizeKb[i];
	}
	if(srcData->ulNandRomfilePartOfsKb != destData->ulNandRomfilePartOfsKb)
		destData->ulNandRomfilePartOfsKb = srcData->ulNandRomfilePartOfsKb;
	if(srcData->ulNandRomfilePartSizeKb != destData->ulNandRomfilePartSizeKb)
		destData->ulNandRomfilePartSizeKb = srcData->ulNandRomfilePartSizeKb;
	if(srcData->ulNandRomdPartOfsKb != destData->ulNandRomdPartOfsKb)
		destData->ulNandRomdPartOfsKb = srcData->ulNandRomdPartOfsKb;
	if(srcData->ulNandRomdPartSizeKb != destData->ulNandRomdPartSizeKb)
		destData->ulNandRomdPartSizeKb = srcData->ulNandRomdPartSizeKb;
	if(srcData->ulNandWwanPartOfsKb != destData->ulNandWwanPartOfsKb)
		destData->ulNandWwanPartOfsKb = srcData->ulNandWwanPartOfsKb;
	if(srcData->ulNandWwanPartSizeKb != destData->ulNandWwanPartSizeKb)
		destData->ulNandWwanPartSizeKb = srcData->ulNandWwanPartSizeKb;
    ...
    int should_yield )
{
    ...
    if( !IS_ERR_OR_NULL(mtd0) )
    {
        ...
        else
        {
           ...
           if( (wt.wfiFlags&WFI_FLAG_HAS_PMC) == 0 )
           {
            ...
            // Only backup needed items.
            backupNVRAM((PNVRAM_DATA)inMemNvramData_buf, pnd);
            spin_unlock_irqrestore(&inMemNvramData_spinlock, flags);
	 ...
	 * the file to be updated is secram.000 and not cferam.000
        ...
        if( sts )
        {
			...
			/*  ZyXEL, try to recover here. We need to update nvram data after.
			 *  Jerry Lin
			 */
			if (!should_yield)
			{
...
{
    return(kerSysSetBootImageStateSeq( newState, kerSysGetSequenceNumber(1), kerSysGetSequenceNumber(2)));
}

int kerSysSetBootImageStateSeq( int newState, int seq1, int seq2 )
{
    int ret = -1;
    int incSeqNumPart = -1;
...
int kerSysNandEraseBlk( void *mtd_ptr )
{
	int blk_addr=0,ret=-1;

	if ( mtd_ptr != NULL ){
		struct mtd_info * mtd = (struct mtd_info *)mtd_ptr;

		for( blk_addr = 0; blk_addr < mtd->size; blk_addr += mtd->erasesize ){
			ret = nandEraseBlk( mtd, blk_addr );
			//printk("erase blk_addr=0x%08x, ret=%d\n",blk_addr,ret);
		}
	}
