...
#if defined(CONFIG_BCM963381) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM96848)
#include "bcm_otp.h"
#endif
...
#endif
//20130822, TimLiu process WiFi button.
#if defined(ZYXEL) && (ZYXEL==1)
#define RESET_BTN_LEGACY            1
#define RESET_BTN_SHORT             2
#define RESET_BTN_LONG              3
#define RESET_EVENTS_BTN_PRESSED	0x00000100
#define RESET_EVENT_BTN_SHORT       0x00000200
#define RESET_EVENT_BTN_LONG        0x00000400
#define RESET_EVENTS                RESET_EVENTS_BTN_PRESSED /*OR all values if any*/
#define WIFI_BTN_LEGACY            1
#define WIFI_BTN_SHORT             2
#define WIFI_BTN_LONG              3
#define WIFI_EVENT_BTN_PRESSED      0x00000010
#define WIFI_EVENT_BTN_SHORT        0x00000020
#define WIFI_EVENT_BTN_LONG         0x00000040
#define WIFI_EVENTS                 WIFI_EVENT_BTN_PRESSED /*OR all values if any*/
//20140103, Timliu +++ add for DECT button
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
#define DECT_BTN_LEGACY            1
#define DECT_BTN_SHORT             2
#define DECT_BTN_LONG              3
#define DECT_EVENT_BTN_PRESSED      0x00001000
#define DECT_EVENT_BTN_SHORT        0x00002000
#define DECT_EVENT_BTN_LONG         0x00004000
#define DECT_EVENTS                 DECT_EVENT_BTN_PRESSED /*OR all values if any*/
#endif
//20140103, Timliu ---
static DEFINE_SPINLOCK(wifiBtn_newapi_spinlock);
static irqreturn_t wifiBtn_isr(int irq, void *dev_id);
static Bool wifiBtn_pressed(void);
#if defined(WIRELESS)
static unsigned int wifiBtn_poll(struct file *file, struct poll_table_struct *wait);
static ssize_t wifiBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos);
#endif
static void wifiBtn_timer_handler(unsigned long arg);
static int __init wifiBtn_mapIntr(int context);
static void __init wifi_board_init(void);
static void __exit wifi_board_deinit(void);
//20130926, Timliu +++ add for reset button
static DEFINE_SPINLOCK(resetBtn_newapi_spinlock);
static Bool resetBtn_pressed(void);
static irqreturn_t resetBtn_isr(int irq, void *dev_id);
static int __init resetBtn_mapIntr(int context);
static void resetBtn_timer_handler(unsigned long arg);
static unsigned int resetBtn_poll(struct file *file, struct poll_table_struct *wait);
static ssize_t resetBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos);
static void __init reset_board_init(void);
static void __exit reset_board_deinit(void);
//20130926, TimLiu ---
//20140103, Timliu +++ add for DECT button
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
static DEFINE_SPINLOCK(dectBtn_newapi_spinlock);
static Bool dectBtn_pressed(void);
static irqreturn_t dectBtn_isr(int irq, void *dev_id);
static int __init dectBtn_mapIntr(int context);
static void dectBtn_timer_handler(unsigned long arg);
static unsigned int dectBtn_poll(struct file *file, struct poll_table_struct *wait);
static ssize_t dectBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos);
static void __init dect_board_init(void);
static void __exit dect_board_deinit(void);
//20140103, TimLiu ---
#endif  //defined(ZYXEL_EXTERNAL_DECT)
#if defined(ZYPRJ_VMG8823_B10B)||defined(ZYPRJ_VMG8823_B30B)||defined(ZYPRJ_VMG8823_B50B)||defined(ZYPRJ_VMG8823_B60B)
#define ZYPRJ_VMG8823_BX0B
#endif

#if defined(ZYXEL_FEATURE_BITS)
unsigned int zy_feature_bits; //global
char mrd_feature_bits[NVRAM_FEATUREBITS_LEN];//global read from MRD
/* Declare based feature bits for each project */
static ZYPROJECT_FEATUREBITS_TABLE zyBasedFeatureBitsTbl[] = {
#if defined(CONFIG_BCM963138)
    {"VMG8823-B10B", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x103041
    {"VMG8823-B30B", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x103042
    {"VMG8823-B50B", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x1030C1
    {"VMG8823-B60B", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x1030C2
#if defined(ZYPRJ_VMG3927_B50B)
    {"VMG3927-B50B", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G)}, //0x30C1
    {"XMG3927-B50B", (ZYFEATUREBITS_ADSL_ANNEXA |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B | ZYFEATUREBITS_VDSL_GFAST |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G)}, //0x31C1
#endif
#if defined(ZYPRJ_VMG8825_B50B)
    {"VMG8825-B50B", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x1030C1
    {"VMG8825-B60B", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)}, //0x1030C2
#endif
#if defined(ZYPRJ_XMG8825_B50A) //(V/X)MG3927,(V/X)MG8825 series
    {"VMG3927-B50A", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G)},
    {"VMG3927-B60A", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G)},
    {"XMG3927-B50A", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B | ZYFEATUREBITS_VDSL_GFAST |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G)},
    {"VMG8825-B50A", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)},
    {"VMG8825-B60A", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP)},
    {"XMG8825-B50A", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A | ZYFEATUREBITS_VDSL_35B | ZYFEATUREBITS_VDSL_GFAST |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G |
                      ZYFEATUREBITS_VOIP |
                      ZYFEATUREBITS_ZIGBEE)},
#endif
#elif defined(CONFIG_BCM963268)
    {"VMG3925-B10C", (ZYFEATUREBITS_ADSL_ANNEXA  |
                      ZYFEATUREBITS_VDSL_17A |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G | ZYFEATUREBITS_WIFI_24G_FEM)}, //0x7041
    {"VMG3925-B30C", (ZYFEATUREBITS_ADSL_ANNEXB |
                      ZYFEATUREBITS_VDSL_17A |
                      ZYFEATUREBITS_WIFI_24G | ZYFEATUREBITS_WIFI_5G | ZYFEATUREBITS_WIFI_24G_FEM)}, //0x7042
#endif
    {"NULL", 0x0}
};
static int _zyHwWifi24GFemCheck(void);
static int _zyHwWifi24GExtAntCheck(void);
static int _zyHwWifi5GFemCheck(void);
static int _zyHwWifi5GExtAntCheck(void);
static int _zyHwVoipSlicCheck(void);
static int _zyMrdFeatureBitsGet(char *MRDfsStr);
static void _zyZyxelFeatureBitsGenerate(void);
#endif
...
static int proc_get_param_string(char *page, char **start, off_t off, int cnt, int *eof, void *data);
...
static int proc_get_zyxel_voip_support(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#if defined(ZYXEL_FEATURE_BITS)
/*!
 *  Get Zyxel Feature Bits from /proc/zyxel/zy_feature_bits.
 *
 *  @note        It's a proc_dir_entry,
 *               kernel command is "$cat /proc/zyxel/zy_feature_bits".
 */
static int proc_get_zyxel_zy_feature_bits(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#endif
#if defined(ZYXEL_SFP_MODULE_SUPPORT)
#if defined(ZYXEL_SFP_DETECT_PIN)
static int proc_get_sfp_present(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#endif
static int proc_get_sfp_ae_los(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#if defined(ZYXEL_SFP_TX_FAULT_PIN)
static int proc_get_sfp_tx_fault(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#endif
#if defined(ZYXEL_SFP_TX_DISABLE)
static int proc_get_sfp_tx_disable(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#endif
#endif
#if defined(ZYXEL_ZIGBEE_SUPPORT)
static int proc_get_zigbee_enable(char *buf, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_set_zigbee_enable(struct file *f, const char *buf, unsigned long cnt, void *data);
static int proc_get_zigbee_reset(char *buf, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_set_zigbee_reset(struct file *f, const char *buf, unsigned long cnt, void *data);
static int proc_get_zigbee_stop_btl(char *buf, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_set_zigbee_stop_btl(struct file *f, const char *buf, unsigned long cnt, void *data);
#endif
#if defined(ZYXEL_BOARD_DETECT_BY_GPIO)
unsigned char zy_board_detect; //global
/*!
 *  Detec board by gpio.
 *
 *  @note        Read each gpio value to set board_detect.
 */
static unsigned char _zyBoardDetectByGpio(void);
/*!
 *  Get board_detect value.
 *
 *  @note       It's a proc_dir_entry, kernel command is "$cat /proc/zyxel/board_detect".
 */
static int proc_board_detect_by_gpio(char *page, char **start, off_t off, int cnt, int *eof, void *data);
#endif
#endif

#if !defined(ZYXEL) || defined(ZYXEL_KERNEL_RESET_BTN)
// Disable reset button function in kernel space.
// Reset target or reset to default is decide by user space.
...
//20130822, TimLiu process WiFi button.
#if defined(ZYXEL) && (ZYXEL==1)
unsigned long gWifiBtnEvOutstanding = 0;
unsigned long gLastWifiBtnEvTime;
static unsigned short wifiBtn_irq = BP_NOT_DEFINED;
static unsigned short wifiBtn_gpio = BP_NOT_DEFINED;
static unsigned short wifiBtn_polling = 0;
static struct timer_list wifiBtn_timer;
static atomic_t wifiBtn_active;
static atomic_t wifiBtn_forced;
//20130926, Timliu +++ add for reset button
unsigned long gResetBtnEvOutstanding = 0;
unsigned long gLastResetBtnEvTime;
static unsigned short resetBtn_irq = BP_NOT_DEFINED;
//static unsigned short resetBtn_gpio = BP_NOT_DEFINED;
static unsigned short resetBtn_polling = 0;
static struct timer_list resetBtn_timer;
static atomic_t resetBtn_active;
static atomic_t resetBtn_forced;
//20130926, TimLiu ---
//20140103, Timliu +++ add for DECT button
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
unsigned long gDectBtnEvOutstanding = 0;
unsigned long gLastDectBtnEvTime;
static unsigned short dectBtn_irq = BP_NOT_DEFINED;
static unsigned short dectBtn_gpio = BP_NOT_DEFINED;
static unsigned short dectBtn_polling = 0;
static struct timer_list dectBtn_timer;
static atomic_t dectBtn_active;
static atomic_t dectBtn_forced;
#endif
//20140103, TimLiu ---
static int g_ZyFwInfoInitialized = 0;
#endif

//20130821,TimLiu Support USB LED, porting from SBG3600
#if defined(ZYXEL) && (ZYXEL==1)
...
void USB_LED_Control( int USBId, int status ) {
	BOARD_LED_NAME led_name = kLedUSB0;
	int led_status = 0;
	if ( USBId == 0x01 ) {
		led_name = kLedUSB0;
	}
	...
	if ( status == 0x01 ) {
		setUsbDeviceInserted(led_name, 1);
		led_status = 1;
	} else if ( status == 0x00 ) {
		setUsbDeviceInserted(led_name, 0);
		led_status = 0;
	}else if ( status == 0x02 )
		led_status = kLedStateFastBlinkContinues;

	kerSysLedCtrl(led_name, led_status);
}
...
#if defined(ZYXEL) && (ZYXEL == 1)
	if( BpGetWirelessBtnExtIntr(&intr) == BP_SUCCESS )
		set_ext_irq_info(intr);
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
	if( BpGetDECTExtIntr(&intr) == BP_SUCCESS )
		set_ext_irq_info(intr);
#endif
#if defined(CONFIG_I2C_SX9310)
	if( BpGetSensorEventExtIntr(&intr) == BP_SUCCESS )
		set_ext_irq_info(intr);
#endif
#endif
...
#if defined(ZYXEL_ZIGBEE_SUPPORT)
//for minimum reset assertion time of 100ns.
static int reset_zigbee_module( void )
{
    unsigned short bpGpio;

    if(BpGetZigbeeResetGpio(&bpGpio) == BP_SUCCESS)
    {
        //printk("Do reset Zigbee module.\n");
...
static void kerSysTargetBoardInit( void )
{
    unsigned short bpGpio;

    bpGpio = (unsigned short)0; //just for passing compile warning

#if defined(ZYXEL_SFP_DETECT_PIN)
	if(BpGetSFPDetectGpio(&bpGpio) == BP_SUCCESS)
	{
		printk("SFP Detect gpio(%x) need to switch to input.\n",bpGpio);
		kerSysSetGpioDirInput(bpGpio);
	}
#endif
#if defined(ZYXEL_SFP_TX_DISABLE)
    if(BpGetSFPTXDisableGpio(&bpGpio) == BP_SUCCESS)
    {
		printk("SFP TxDisable gpio(%x) need to switch to output.\n",bpGpio);
		kerSysSetGpioDir(bpGpio);
		printk("Default set to Inactive\n");
		kerSysSetGpioState(bpGpio,kGpioInactive);
    }
#endif
#if (defined(ZYPRJ_XMG3512)||defined(ZYPRJ_EMG3415)||defined(ZYPRJ_VMG8823_BX0B)||defined(ZYPRJ_XMG3563_B10A)||defined(ZYPRJ_VMG8825_B50B))
	if(BpGetSelectWanLedGpio(&bpGpio) == BP_SUCCESS)
	{
#if (defined(ZYPRJ_VMG8823_BX0B) || defined(ZYPRJ_EMG3415) || defined(ZYPRJ_VMG8825_B50B))
		printk("Default output WAN signal to WAN led.\n");
		kerSysSetGpioState(bpGpio,kGpioInactive);
#else
		printk("Default output WAN signal to SFP led.\n");
		kerSysSetGpioState(bpGpio,kGpioActive);
#endif
	}
#endif
#if defined(ZYPRJ_VMG4927_B50A) || defined(ZYPRJ_VMG8825_B50K) || defined(ZYPRJ_EMG6726_B10A)
    {   /* 20170927 For running on C3000Z board */
...
#endif
#if defined(ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED)
    {   /* 20171023 ex. VMG8825-B50B_WIND used */
        if (BpGetLanAggregateLinkGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("LAN Aggregate Link (%d) need to config to output.\n",bpGpio);
            kerSysSetGpioDir(bpGpio);
            printk("Default set to Inactive\n");
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
        if (BpGetLanAggregateActGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("LAN Aggregate Ack (%d) need to config to output.\n",bpGpio);
            kerSysSetGpioDir(bpGpio);
            printk("Default set to Inactive\n");
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
...
#endif
#if defined(ZYPRJ_VMG8825_B50B) || defined(ZYPRJ_VMG3927_B50B) || defined(ZYPRJ_VMG8825_B50K) || defined(ZYPRJ_XMG8825_B50A)
    {
        if (BpGet24GDisableGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("WiFi 2.4G disable pin (%d) need to config to Inactive.\n",bpGpio);
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
        if (BpGet5GDisableGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("WiFi 5G disable pin (%d) need to config to Inactive.\n",bpGpio);
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
        if (BpGet24GResetGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("WiFi 2.4G reset pin (%d) need to config to Inactive.\n",bpGpio);
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
        if (BpGet5GResetGpio(&bpGpio) == BP_SUCCESS)
        {
            printk("WiFi 5G reset pin (%d) need to config to Inactive.\n",bpGpio);
            kerSysSetGpioState(bpGpio, kGpioInactive);
        }
#if defined(ZYXEL_ZIGBEE_SUPPORT)
        if(BpGetZigbeeStopBtlGpio(&bpGpio) == BP_SUCCESS)
        {
    	    printk("Zigbee StopBtl gpio(%x) need to switch to output.\n",bpGpio);
    	    kerSysSetGpioDir(bpGpio);
			kerSysSetGpioState(bpGpio, kGpioInactive);
        }
        if(BpGetZigbeePwrEnGpio(&bpGpio) == BP_SUCCESS)
        {
    	    printk("Zigbee PwrEn gpio(%x) need to switch to output.\n",bpGpio);
    	    kerSysSetGpioDir(bpGpio);
			printk("Default enable the power for Zigbee module.\n");
			kerSysSetGpioState(bpGpio, kGpioActive);
        }
        if(BpGetZigbeeResetGpio(&bpGpio) == BP_SUCCESS)
        {
    	    printk("Zigbee reset gpio(%x) need to switch to output.\n",bpGpio);
    	    kerSysSetGpioDir(bpGpio);
			if( !reset_zigbee_module() )
			{
				printk("Fail to reset zigbee module.\n");
			}
        }
    ...
    {
        int tb_index;
        zy_board_detect = _zyBoardDetectByGpio();
        tb_index = (int)zy_board_detect;
        printk("Board detect status = %u (%u, %s).\n",zy_board_detect, zyBoardDetectTable[tb_index].value, zyBoardDetectTable[tb_index].szBoardId);
    }
#endif
}
#endif
...
//20130822,TimLiu Support USB LED, porting from SBG3600
#if defined(ZYXEL) && (ZYXEL==1)
        init_timer(&gUsbTimer);
        gUsbTimer.function = (void*)UsbTimerExpire;
        gUsbTimer.expires = jiffies+ktimems_v;        // timer expires in ~100ms
...
//20130822, TimLiu enable WiFi button IRQ.
#if defined(ZYXEL) && (ZYXEL == 1)
        wifi_board_init();
        reset_board_init();
//20140103, Timliu +++ add for DECT button
	...
	{
		unsigned short bpGpio;
		//read out the board parameter to double check the status.
		if(BpGetHTPGpio(&bpGpio) == BP_SUCCESS)
		{
			printk("HTP was config to use gpio %04x.\n",bpGpio);
		}else
			printk("Not config HTP.\n");
	}
#endif
#endif
...
#endif //!defined(ZYXEL) || defined(ZYXEL_KERNEL_RESET_BTN)
    ...
    registerBtns();
#if defined(ZYXEL) && (ZYXEL==1) && defined(ZYXEL_FEATURE_BITS)
    _zyZyxelFeatureBitsGenerate();
#endif
    add_proc_files();
...
void __exit brcm_board_cleanup( void )
{
    ...
    if (board_major != -1)
    {
        ...
        ses_board_deinit();
#endif
//20130822, TimLiu disable WiFi IRQ.
#if defined(ZYXEL) && (ZYXEL==1)
        wifi_board_deinit();
        reset_board_deinit();
//20140103, Timliu +++ add for DECT button
...
#if defined(ZYXEL) && (ZYXEL == 1)
	else if(board_ioc->eventmask & WIFI_EVENTS){
		mask |= wifiBtn_poll(filp, wait);
	}
#endif	//#if define(ZYXEL)
#endif  //defined (WIRELESS)
#if defined(ZYXEL) && (ZYXEL == 1)
	else if(board_ioc->eventmask & RESET_EVENTS){
        mask |= resetBtn_poll(filp, wait);
    }
//20140103, Timliu +++ add for DECT button
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
	else if(board_ioc->eventmask & DECT_EVENTS){
		mask |= dectBtn_poll(filp, wait);
	}
#endif	//ZYXEL_EXTERNAL_DECT
#endif
...
static ssize_t board_read(struct file *filp,  char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if(board_ioc->eventmask & SES_EVENTS){
        ...
        return wifiBtn_read(filp, buffer, count, ppos);
    }
#endif	//#if define(ZYXEL)
#endif  //defined (WIRELESS)
#if defined(ZYXEL) && (ZYXEL == 1)
	else if(board_ioc->eventmask & RESET_EVENTS){
        return resetBtn_read(filp, buffer, count, ppos);
    }
//20140103, Timliu +++ add for DECT button
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
	else if(board_ioc->eventmask & DECT_EVENTS){
        return dectBtn_read(filp, buffer, count, ppos);
    }
#endif  //#if defined(ZYXEL_EXTERNAL_DECT)
#endif
    return 0;
}
...
#if defined(ZYXEL) && defined(ZYXEL_QTN_SW_RESET)
void resetQtnModule(void)
{
	unsigned short gpio;
	if ( BpGet5GResetGpio(&gpio) == BP_SUCCESS ) {
		kerSysSetGpioState(gpio, kGpioInactive);
		kerSysSetGpioState(gpio, kGpioActive);
		//reference QTM840-11ac-RGMII_P-HW Spec v1.2-Confidential_Zyxel.pdf, P.8
		mdelay(1); //delay 1ms is enough
		kerSysSetGpioState(gpio, kGpioInactive);
		printk(KERN_INFO "resetQtnModule: reset QTN module\n");
	}
}
#endif
...
int kerSysGetMacAddress( unsigned char *pucaMacAddr, unsigned long ulId )
{
	...
	if (i == 3){	//Don't use 4th MAC
        ...
        )
        {
            /* This MAC address has been used by the caller in the past. */
#ifdef ZYXEL_L2_WAN_SAME_MAC
#ifdef ZYXEL_L3_WAN_SAME_MAC
        if( CHECK_WANID(pMai->ulId, ulId) && i == 4 ) {
#else
	    if( CHECK_WANID(pMai->ulId, ulId) && i == 4 && ((ulId & 0x0000000FL) == 0x00000001L) ){
#endif
		continue;		//The 5th MAC is L2 WAN MAC, don't assign to L3 WAN interface
	    }else if ( CHECK_WANID(pMai->ulId, ulId) ) {
                pMai->ulId = ulId;
            }
    ...
    if( pMaiFreeNoId || pMaiFreeId )
    {
        ...
        if( pMaiFreeNoId )
        {
            getNthMacAddr(pucaMacAddr, ulIdxNoId);
...
int kerSysGetMacAddressByIndex( unsigned char *pucaMacAddr, unsigned int index )
{
	int nRet = 0;
...
static int commonImageWrite(int flash_start_addr, char *string, int size,
    int *pnoReboot, int partition, NVRAM_DATA *nvramFwInfo)
#else
static int commonImageWrite(int flash_start_addr, char *string, int size,
...
#if defined(ZCFG_USED)
			printk(KERN_EMERG "Set Broadcom Nand Image Start...\n");
#endif
            ret = kerSysBcmNandImageSet(rootfs_part, string, size,
                (pnoReboot) ? *pnoReboot : 0);
#if defined(ZCFG_USED)
			printk(KERN_EMERG "Set Broadcom Nand Image Finished\n");
#endif
        }
...
#if 0//defined(ZCFG_USED)
// Disable update below items in NVRAM when doing upgrade firmware
		else {
			/*ZyXEL, Update firmware information after writing firmware image. Jerry Lin*/
			if(nvramFwInfo != NULL) {
				memcpy(pNvramDataNew->FirmwareVersion, nvramFwInfo->FirmwareVersion, NVRAM_FIRMWAREVERSION_LEN);
				pNvramDataNew->ulKernelChecksum = nvramFwInfo->ulKernelChecksum;
				pNvramDataNew->ulRootFSChecksum = nvramFwInfo->ulRootFSChecksum;
				memcpy(pNvramDataNew->ProductName, nvramFwInfo->ProductName, NVRAM_PRODUCTNAME_LEN);
#if 0
				printk(KERN_EMERG "%s : FirmwareVersion    %s\n", __FUNCTION__, pNvramDataNew->FirmwareVersion);
				printk(KERN_EMERG "%s : KernelChecksum     %u\n", __FUNCTION__, pNvramDataNew->ulKernelChecksum);
				printk(KERN_EMERG "%s : RootFSChecksum     %u\n", __FUNCTION__, pNvramDataNew->ulRootFSChecksum);
				printk(KERN_EMERG "%s : ProductName        %s\n", __FUNCTION__, pNvramDataNew->ProductName);
#endif
				writeNvramDataCrcLocked(pNvramDataNew);
			}
		}
#endif
    }
...
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1) || defined(ZYXEL_WIFI_LED_PATTERN) && (ZYXEL_WIFI_LED_PATTERN == 1)
    short bpGpio;
#endif
        ...
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case BCM_IMAGE_CFE:
				...
				ret = commonImageWrite(ctrlParms.offset + BOOT_OFFSET + IMAGE_OFFSET,
                    ctrlParms.string, ctrlParms.strLen, NULL, 0, NULL);
#else
                ret = commonImageWrite(ctrlParms.offset + BOOT_OFFSET + IMAGE_OFFSET,
            ...
            case BCM_IMAGE_WHOLE:
                ...
                int partition = FLASH_GET_PARTITION(ctrlParms.offset);
#if defined(ZCFG_USED)
				NVRAM_DATA * pNvramData = (NVRAM_DATA *)ctrlParms.buf;
#endif
#if defined(CONFIG_BCM_WATCHDOG_TIMER)
                int resumeWD;
#endif
				...
				ret = commonImageWrite(FLASH_BASE, ctrlParms.string,
                    ctrlParms.strLen, &noReboot, partition, pNvramData);
#else
                ret = commonImageWrite(FLASH_BASE, ctrlParms.string,
...
//20140103, Timliu +++ add for DECT button
    ...
    case BOARD_IOCTL_GET_GPIO:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
        	ctrlParms.result = kerSysGetGpioValue((unsigned)ctrlParms.strLen);
            __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
            ret = 0;
        }
        else {
...
#if defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
    case BOARD_IOCTL_SET_DECT_RESET:
    	// set "1" to reset DECT, "0" is normal state.
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
        	if(BpGetDECTResetGpio(&bpGpio) == BP_SUCCESS) {
        	    kerSysSetGpioState(bpGpio, ctrlParms.strLen);
        	    __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
        	    ret = 0;
        	}else {
        		printk("DECT reset gpio is not defined.\n");
        		ret = -EFAULT;
        	}
        }
        else {
	...
	case BOARD_IOCTL_SET_WIRELESS_SELECT:
		// low for 2.4G WiFi LED, hight for 5G WiFi LED
	    if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
        	if(BpGetWirelessSelectGpio(&bpGpio) == BP_SUCCESS) {
        		...
        		printk("Wireless select gpio is not defined.\n");
        		ret = -EFAULT;
        	}
        }
        else {
    	...
    	break;
#endif
	case BOARD_IOCTL_ERASE_PARTITION:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
			struct mtd_info *mtd;
            switch (ctrlParms.action) {
				case ERASE_PAR_DATA:
					...
					if( !IS_ERR_OR_NULL(mtd) )
						ret = kerSysNandEraseBlk(mtd);
					break;

				case ERASE_PAR_ROMD:
					mtd = get_mtd_device_nm("rom-d");
					if( !IS_ERR_OR_NULL(mtd) )
						ret = kerSysNandEraseBlk(mtd);
					break;
            ...
            ret = -EFAULT;
		break;
    case BOARD_IOCTL_GET_ZYFWINFO:
		if(!g_ZyFwInfoInitialized) {
			kerSysZyFwInfoLoad();
			g_ZyFwInfoInitialized=1;
		}
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
			kerSysZyFwInfoGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
			ret = 0;
        	ctrlParms.result = ret;
            __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
        }
    ...
    case BOARD_IOCTL_BOOT_IMAGE_OPERATION:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case BOOT_SET_NEW_IMAGE_ONCE:
                if (ctrlParms.strLen == -10)
                    ctrlParms.result = kerSysSetBootImageState(ctrlParms.offset);
                else
                    ctrlParms.result = kerSysSetBootImageStateSeq(ctrlParms.offset, (int) ctrlParms.action, ctrlParms.strLen);
                /* Clean up image version. */
                if (ctrlParms.result == 0)
                {
                    memset(&imageVersions, 0, sizeof(imageVersions));
...
static Bool sesBtn_pressed(void)
{
	...
	if( sesBtn_polling == 0 )
	{
...
#if defined(ZYPRJ_VMG8948)
		BpGetWirelessSesExtIntrGpio(&bpGpio);
		gpioActHigh = (bpGpio & BP_ACTIVE_LOW) ? 0 : 1;
#endif
		...
		if( ( actHigh && intSts ) || (!actHigh && !intSts ) )
		{
			...
			//printk("%s(%d): actHigh(%d), intSts(%d), value(%d)\n",__func__,__LINE__,actHigh,intSts,value);
		}
		else
			pressed = 0;
	}
...
static irqreturn_t sesBtn_isr(int irq, void *dev_id)
{
...
#if defined(ZYPRJ_VMG8948)
		BpGetWirelessSesExtIntrGpio(&bpGpio);
		if( ((bpGpio & BP_ACTIVE_LOW) && !value) || (!(bpGpio & BP_ACTIVE_LOW) && value) )
#else
	...
	//printk("sesBtn_isr: triggered(%d).\n", IRQ_HANDLED);	//timliu debug
    if (IRQ_HANDLED == ret) {   
...
//20130822, TimLiu detect WiFi button status
#if defined(ZYXEL) && (ZYXEL==1)
static Bool wifiBtn_pressed(void)
{
		unsigned int intSts = 0, extIntr, value = 0;
		...
		if( wifiBtn_polling == 0 )
		{
#if defined(CONFIG_BCM96838)
			if ((wifiBtn_irq >= INTERRUPT_ID_EXTERNAL_0) && (wifiBtn_irq <= INTERRUPT_ID_EXTERNAL_5)) {
#else
			if ((wifiBtn_irq >= INTERRUPT_ID_EXTERNAL_0) && (wifiBtn_irq <= INTERRUPT_ID_EXTERNAL_3)) {
#endif

#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
				intSts = kerSysGetGpioValue(MAP_EXT_IRQ_TO_GPIO( wifiBtn_irq - INTERRUPT_ID_EXTERNAL_0));
#elif defined(CONFIG_BCM963381) || defined(CONFIG_BCM96848)
				intSts = PERF->ExtIrqSts & (1 << (wifiBtn_irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#else
				intSts = PERF->ExtIrqCfg & (1 << (wifiBtn_irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#endif
			}
			else
				return 0;

			extIntr = extIntrInfo[wifiBtn_irq -INTERRUPT_ID_EXTERNAL_0];
#if defined(ZYPRJ_VMG8948)
			BpGetWirelessBtnExtIntrGpio(&bpGpio);
			gpioActHigh = (bpGpio & BP_ACTIVE_LOW) ? 0 : 1;
#endif
			actHigh = IsExtIntrTypeActHigh(extIntr);
			...
			if( ( actHigh && intSts ) || (!actHigh && !intSts ) )
			{
				...
				if( IsExtIntrShared(extIntr) )
				{
					 value = kerSysGetGpioValue(wifiBtn_gpio);
#if defined(ZYPRJ_VMG8948)
					 if( (value&&!gpioActHigh) || (!value&&gpioActHigh) )
 #else
 					 if( (value&&!actHigh) || (!value&&actHigh) )
		...
		{
			pressed = 0;
			if( wifiBtn_gpio != BP_NOT_DEFINED )
			{
				actHigh = wifiBtn_gpio&BP_ACTIVE_LOW ? 0 : 1;
				value = kerSysGetGpioValue(wifiBtn_gpio);
...
static unsigned int wifiBtn_poll(struct file *file, struct poll_table_struct *wait)
{
    ...
    // occured within the last second.   Otherwise, we fall through to check the
    // original checks.
    if (gWifiBtnEvOutstanding) {
        if (time_after(gLastWifiBtnEvTime+HZ, jiffies)) {
            return POLLIN;
        } else {
            atomic_set(&wifiBtn_active, 0);
            gWifiBtnEvOutstanding = 0;
        }
    }
    if ( wifiBtn_polling ) {
        if ( wifiBtn_pressed() ) {
	     ...
	     else if (atomic_read(&wifiBtn_active)) {
        return POLLIN;
    }
    else if (atomic_read(&wifiBtn_forced)) {
        atomic_set(&wifiBtn_forced,0);
        return POLLIN;
    }
    return 0;
}
...
static ssize_t wifiBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    int exit = 0;
     ...
     /* Synchronization note: This code does a non-atomic test and set of
     * wifiBtn_active that could cause a race-condition with btnHook_Wifi,
     * so this must be protected with wifiBtn_newapi_spinlock.
     */
    /* New button API: Return the type of WiFi button press (Short/Long) */
    spin_lock_irqsave(&wifiBtn_newapi_spinlock, flags);
    if (atomic_read(&wifiBtn_active) == WIFI_BTN_SHORT) {
        event = WIFI_EVENTS | WIFI_EVENT_BTN_SHORT;
        atomic_set(&wifiBtn_active, 0);
    }
    else if (atomic_read(&wifiBtn_active) == WIFI_BTN_LONG) {
        event = WIFI_EVENTS | WIFI_EVENT_BTN_LONG;
        atomic_set(&wifiBtn_active, 0);
    }
    /* Legacy button API: Return a simple flag (WIFI_EVENTS) and let the
     * userspace code call read repeatedly to calculate the press time
     */
    else {
		if ( wifiBtn_polling ) {
			if ( 0 == wifiBtn_pressed() ) {
				exit = 1;
			}
		}
		else if ( 0 == atomic_read(&wifiBtn_active) ) {
			exit = 1;
		}
		event = WIFI_EVENTS;
	}
	spin_unlock_irqrestore(&wifiBtn_newapi_spinlock, flags);
    if (exit)
        return ret;
	gWifiBtnEvOutstanding = 0;
...
static void wifiBtn_timer_handler(unsigned long arg)
{
    unsigned long currentJiffies = jiffies;
    if ( wifiBtn_pressed() ) {
        doPushButtonHold(PB_BUTTON_2, currentJiffies);
        mod_timer(&wifiBtn_timer, currentJiffies + msecs_to_jiffies(100));
    }
	else {
        atomic_set(&wifiBtn_active, 0);
        doPushButtonRelease(PB_BUTTON_2, currentJiffies);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqUnmask(wifiBtn_irq);
#else
        BcmHalInterruptEnable(wifiBtn_irq);
#endif
     }
}

void wifiBtn_defaultAction(unsigned long time, unsigned long param)
{
    wake_up_interruptible(&g_board_wait_queue);
}
...
static irqreturn_t wifiBtn_isr(int irq, void *dev_id)
{
	...
	//printk("wifiBtn_isr: triggered(%d).\n", IRQ_HANDLED);	//timliu debug

	if (IRQ_HANDLED == ret)
	{
		int timerSet = mod_timer(&wifiBtn_timer, (currentJiffies + msecs_to_jiffies(100)));	/* 100 msec */
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
		BcmHalExternalIrqMask(irq);
#endif
		if ( 0 == timerSet )
		{
			atomic_set(&wifiBtn_active, WIFI_BTN_LEGACY);
			doPushButtonPress(PB_BUTTON_2, currentJiffies);
		}
	}
...
static int __init wifiBtn_mapIntr(int context)
{
    ...
    if( BpGetWirelessBtnExtIntr(&wifiBtn_irq) == BP_SUCCESS )
    {
        BpGetWirelessBtnExtIntrGpio(&wifiBtn_gpio);
        if( wifiBtn_irq != BP_EXT_INTR_NONE )
        {
#if defined(CONFIG_BCM960333)
            if( wifiBtn_gpio != BP_NOT_DEFINED && wifiBtn_irq != BP_EXT_INTR_NONE)
                mapBcm960333GpioToIntr(wifiBtn_gpio & BP_GPIO_NUM_MASK, wifiBtn_irq);
#endif
            printk("WIFI: Button Interrupt 0x%x is enabled\n", wifiBtn_irq);
        }
        else
        {
            if( wifiBtn_gpio != BP_NOT_DEFINED )
            {
                printk("%s(%d): Set GPIO_%x to input...\n", __func__,__LINE__,wifiBtn_gpio);
                kerSysSetGpioDirInput(wifiBtn_gpio);
                wifiBtn_polling = 1;
            }
        }
    }
    ...
    if( wifiBtn_irq != BP_EXT_INTR_NONE )
    {
		ext_irq_idx = (wifiBtn_irq&~BP_EXT_INTR_FLAGS_MASK)-BP_EXT_INTR_0;
#if defined(CONFIG_BCM963381) || defined(CONFIG_BCM96848)
        kerSysInitPinmuxInterface(BP_PINMUX_FNTYPE_IRQ | ext_irq_idx);
#endif
		if (!IsExtIntrConflict(extIntrInfo[ext_irq_idx]))
		{
			...
			if (IsExtIntrShared(wifiBtn_irq))
			{
				/* get the gpio and make it input dir */
				if( wifiBtn_gpio != BP_NOT_DEFINED )
				{
					wifiBtn_gpio &= BP_GPIO_NUM_MASK;
					printk("%s(%d): Set GPIO_%x to input...\n", __func__,__LINE__,wifiBtn_gpio);
					kerSysSetGpioDirInput(wifiBtn_gpio);
					dev = wifiBtn_gpio;
				}
				...
				{
					printk("%s(%d): Button Interrupt gpio definition not found \n",__func__,__LINE__);
					hookisr = 0;
				}
			}
			...
			if(hookisr)
			{
			    init_timer(&wifiBtn_timer);
                wifiBtn_timer.function = wifiBtn_timer_handler;
                wifiBtn_timer.expires  = jiffies + msecs_to_jiffies(100);
                wifiBtn_timer.data     = 0;
                atomic_set(&wifiBtn_active, 0);
                atomic_set(&wifiBtn_forced, 0);
			    wifiBtn_irq = map_external_irq (wifiBtn_irq);
				ret = 1;
			    BcmHalMapInterrupt((FN_HANDLER)wifiBtn_isr, (unsigned int)&dev, wifiBtn_irq);
#if !defined(CONFIG_ARM)
                BcmHalInterruptEnable(wifiBtn_irq);
#endif
			}
...
static void __init wifi_board_init()
{
    int ret;
    ret = wifiBtn_mapIntr(0);
    if (ret) {
		registerPushButtonPressNotifyHook(PB_BUTTON_2, wifiBtn_defaultAction, 0);
	}
}

static void __exit wifi_board_deinit()
{
    if(wifiBtn_polling == 0 && wifiBtn_irq != BP_NOT_DEFINED )
    {
	    int ext_irq_idx = wifiBtn_irq - INTERRUPT_ID_EXTERNAL_0;
        if(wifiBtn_irq) {
            del_timer(&wifiBtn_timer);
            atomic_set(&wifiBtn_active, 0);
            atomic_set(&wifiBtn_forced, 0);
            if (!IsExtIntrShared(extIntrInfo[ext_irq_idx])) {
                BcmHalInterruptDisable(wifiBtn_irq);
            }
        }
    }
}

//20131031, Timliu +++ add for reset button
static Bool resetBtn_pressed(void)
{
	unsigned int intSts = 0, extIntr, value = 0;
	int actHigh = 0;
	...
	if( resetBtn_polling == 0 )
	{
...
#elif defined(CONFIG_BCM963381) || defined(CONFIG_BCM96848)
			intSts = PERF->ExtIrqSts & (1 << (resetBtn_irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#else
		...
		if( ( actHigh && intSts ) || (!actHigh && !intSts ) )
		{
			...
			if( IsExtIntrShared(extIntr) )
			{
				 value = kerSysGetGpioValue(resetBtn_gpio);
				 if( (value&&!actHigh) || (!value&&actHigh) )
					 pressed = 0;
			}
			//printk("%s(%d): actHigh(%d), intSts(%d), value(%d)\n",__func__,__LINE__,actHigh,intSts,value);
	...
	else
	{
		...
		{
			actHigh = resetBtn_gpio&BP_ACTIVE_LOW ? 0 : 1;
			value = kerSysGetGpioValue(resetBtn_gpio);
			if( (value&&actHigh) || (!value&&!actHigh) )
				pressed = 1;
		}
...
static unsigned int resetBtn_poll(struct file *file, struct poll_table_struct *wait)
{
    ...
    if (gResetBtnEvOutstanding) {
        if (time_after(gLastResetBtnEvTime+HZ, jiffies)) {
            return POLLIN;
        } else {
            atomic_set(&resetBtn_active, 0);
            gResetBtnEvOutstanding = 0;
        }
    }
    ...
    else if (atomic_read(&resetBtn_active)) {
        return POLLIN;
    }
    else if (atomic_read(&resetBtn_forced)) {
        atomic_set(&resetBtn_forced,0);
        return POLLIN;
    }
...
static ssize_t resetBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    /* Synchronization note: This code does a non-atomic test and set of
     * resetBtn_active that could cause a race-condition with btnHook_Reset,
     * so this must be protected with resetBtn_newapi_spinlock.
     */

    /* New button API: Return the type of Reset button press (Short/Long) */
    spin_lock_irqsave(&resetBtn_newapi_spinlock, flags);
    if (atomic_read(&resetBtn_active) == RESET_BTN_SHORT) {
        event = RESET_EVENTS | RESET_EVENT_BTN_SHORT;
        atomic_set(&resetBtn_active, 0);
    }
    else if (atomic_read(&resetBtn_active) == RESET_BTN_LONG) {
        event = RESET_EVENTS | RESET_EVENT_BTN_LONG;
        atomic_set(&resetBtn_active, 0);
    }
    /* Legacy button API: Return a simple flag (RESET_EVENTS) and let the
     * userspace code call read repeatedly to calculate the press time
     */
    else {
		if ( resetBtn_polling ) {
			if ( 0 == resetBtn_pressed() ) {
				exit = 1;
			}
		}
		else if ( 0 == atomic_read(&resetBtn_active) ) {
			exit = 1;
		}
		event = RESET_EVENTS;
	}
	spin_unlock_irqrestore(&resetBtn_newapi_spinlock, flags);
...
static void resetBtn_timer_handler(unsigned long arg)
{
    unsigned long currentJiffies = jiffies;
    if ( resetBtn_pressed() ) {
        doPushButtonHold(PB_BUTTON_3, currentJiffies);
        mod_timer(&resetBtn_timer, currentJiffies + msecs_to_jiffies(100));
		//printk("resetBtn_timer_handler: resetBtn_pressed() == true.\n");
    }
    else {
        atomic_set(&resetBtn_active, 0);
        doPushButtonRelease(PB_BUTTON_3, currentJiffies);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqUnmask(resetBtn_irq);
#else
        BcmHalInterruptEnable(resetBtn_irq);
#endif
		//printk("resetBtn_timer_handler: resetBtn_pressed() == false.\n");
    }
}

void resetBtn_defaultAction(unsigned long time, unsigned long param)
{
    wake_up_interruptible(&g_board_wait_queue);
}
...
static irqreturn_t resetBtn_isr(int irq, void *dev_id)
{
	...
	//printk("resetBtn_isr: triggered(%d).\n", IRQ_HANDLED);	//timliu debug

	if (IRQ_HANDLED == ret)
	{
		int timerSet = mod_timer(&resetBtn_timer, (currentJiffies + msecs_to_jiffies(100)));	/* 100 msec */
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqMask(irq);
#endif
		if ( 0 == timerSet ) {
            atomic_set(&resetBtn_active, RESET_BTN_LEGACY);
            doPushButtonPress(PB_BUTTON_3, currentJiffies);
		}
	}
...
static int __init resetBtn_mapIntr(int context)
{
    ...
    {
        BpGetResetToDefaultExtIntrGpio(&resetBtn_gpio);
        if( resetBtn_irq != BP_EXT_INTR_NONE )
        {
#if defined(CONFIG_BCM960333)
            if( resetBtn_gpio != BP_NOT_DEFINED && resetBtn_irq != BP_EXT_INTR_NONE)
                mapBcm960333GpioToIntr(resetBtn_gpio & BP_GPIO_NUM_MASK, resetBtn_irq);
#endif
            printk("RESET: Button Interrupt 0x%x is enabled\n", resetBtn_irq);
        }
        else
        {
            if( resetBtn_gpio != BP_NOT_DEFINED )
            {
                printk("RESET: Button Polling is enabled on gpio %x\n", resetBtn_gpio);
                kerSysSetGpioDirInput(resetBtn_gpio);
                resetBtn_polling = 1;
            }
        }
    }
    ...
    if( resetBtn_irq != BP_EXT_INTR_NONE )
    {
        ...
        if (!IsExtIntrConflict(extIntrInfo[ext_irq_idx]))
        {
            ...
            if (IsExtIntrShared(resetBtn_irq))
            {
                ...
                {
                    resetBtn_gpio &= BP_GPIO_NUM_MASK;;
                    printk("RESET: Button Interrupt gpio is %d\n", resetBtn_gpio);
                    kerSysSetGpioDirInput(resetBtn_gpio);
                    dev = resetBtn_gpio;
                }
                else
                {
                    printk("RESET: Button Interrupt gpio definition not found \n");
                    hookisr = 0;
                }
            }
            ...
            if(hookisr)
            {
                ...
                resetBtn_timer.expires  = jiffies + msecs_to_jiffies(100);
                resetBtn_timer.data     = 0;
                atomic_set(&resetBtn_active, 0);
                atomic_set(&resetBtn_forced, 0);
                resetBtn_irq = map_external_irq (resetBtn_irq);
				ret = 1;
                BcmHalMapInterrupt((FN_HANDLER)resetBtn_isr, (unsigned int)&dev, resetBtn_irq);
#if !defined(CONFIG_ARM)
                BcmHalInterruptEnable(resetBtn_irq);
#endif
            }
...
static void __init reset_board_init()
{
    int ret;
    ret = resetBtn_mapIntr(0);
    if (ret) {
		registerPushButtonPressNotifyHook(PB_BUTTON_3, resetBtn_defaultAction, 0);
	}
}

static void __exit reset_board_deinit()
{
	if(resetBtn_polling == 0 && resetBtn_irq != BP_NOT_DEFINED )
    {
	    int ext_irq_idx = resetBtn_irq - INTERRUPT_ID_EXTERNAL_0;
        if(resetBtn_irq) {
            del_timer(&resetBtn_timer);
            atomic_set(&resetBtn_active, 0);
            atomic_set(&resetBtn_forced, 0);
            if (!IsExtIntrShared(extIntrInfo[ext_irq_idx])) {
                BcmHalInterruptDisable(resetBtn_irq);
            }
        }
    }
}
...
static Bool dectBtn_pressed(void)
{
	...
	if( dectBtn_polling == 0 )
	{
#if defined(CONFIG_BCM96838)
		if ((dectBtn_irq >= INTERRUPT_ID_EXTERNAL_0) && (dectBtn_irq <= INTERRUPT_ID_EXTERNAL_5)) {
#else
		if ((dectBtn_irq >= INTERRUPT_ID_EXTERNAL_0) && (dectBtn_irq <= INTERRUPT_ID_EXTERNAL_3)) {
#endif
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
			intSts = kerSysGetGpioValue(MAP_EXT_IRQ_TO_GPIO( dectBtn_irq - INTERRUPT_ID_EXTERNAL_0));
#elif defined(CONFIG_BCM963381) || defined(CONFIG_BCM96848)
			intSts = PERF->ExtIrqSts & (1 << (dectBtn_irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#else
			intSts = PERF->ExtIrqCfg & (1 << (dectBtn_irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#endif
		}
		else
			return 0;

		extIntr = extIntrInfo[dectBtn_irq -INTERRUPT_ID_EXTERNAL_0];
		actHigh = IsExtIntrTypeActHigh(extIntr);
		...
		if( ( actHigh && intSts ) || (!actHigh && !intSts ) )
		{
			...
			if( IsExtIntrShared(extIntr) )
			{
				 value = kerSysGetGpioValue(dectBtn_gpio);
				 if( (value&&!actHigh) || (!value&&actHigh) )
					 pressed = 0;
			}
			//printk("%s(%d): actHigh(%d), intSts(%d), value(%d)\n",__func__,__LINE__,actHigh,intSts,value);
	...
	{
		pressed = 0;
		if( dectBtn_gpio != BP_NOT_DEFINED )
		{
			actHigh = dectBtn_gpio&BP_ACTIVE_LOW ? 0 : 1;
			value = kerSysGetGpioValue(dectBtn_gpio);
			if( (value&&actHigh) || (!value&&!actHigh) )
				pressed = 1;
		}
...
static unsigned int dectBtn_poll(struct file *file, struct poll_table_struct *wait)
{
    ...
    // occured within the last second.   Otherwise, we fall through to check the
    // original checks.
    if (gDectBtnEvOutstanding) {
        if (time_after(gLastDectBtnEvTime+HZ, jiffies)) {
            return POLLIN;
        } else {
            atomic_set(&dectBtn_active, 0);
            gDectBtnEvOutstanding = 0;
        }
    ...
    else if (atomic_read(&dectBtn_active)) {
        return POLLIN;
    }
    else if (atomic_read(&dectBtn_forced)) {
        atomic_set(&dectBtn_forced,0);
        return POLLIN;
    }
    return 0;
}
...
static ssize_t dectBtn_read(struct file *file,  char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    /* Synchronization note: This code does a non-atomic test and set of
     * dectBtn_active that could cause a race-condition with btnHook_Dect,
     * so this must be protected with dectBtn_newapi_spinlock.
     */

    /* New button API: Return the type of Dect button press (Short/Long) */
    spin_lock_irqsave(&dectBtn_newapi_spinlock, flags);
    if (atomic_read(&dectBtn_active) == DECT_BTN_SHORT) {
        event = DECT_EVENTS | DECT_EVENT_BTN_SHORT;
        atomic_set(&dectBtn_active, 0);
    }
    else if (atomic_read(&dectBtn_active) == DECT_BTN_LONG) {
        event = DECT_EVENTS | DECT_EVENT_BTN_LONG;
        atomic_set(&dectBtn_active, 0);
    }
    /* Legacy button API: Return a simple flag (DECT_EVENTS) and let the
     * userspace code call read repeatedly to calculate the press time
     */
    else {
		if ( dectBtn_polling ) {
			if ( 0 == dectBtn_pressed() ) {
				exit = 1;
			}
		}
		else if ( 0 == atomic_read(&dectBtn_active) ) {
			exit = 1;
		}
		event = DECT_EVENTS;
	}
	spin_unlock_irqrestore(&dectBtn_newapi_spinlock, flags);
...
static void dectBtn_timer_handler(unsigned long arg)
{
    unsigned long currentJiffies = jiffies;
	if ( dectBtn_pressed() ) {
		doPushButtonHold(PB_BUTTON_4, currentJiffies);
        mod_timer(&dectBtn_timer, currentJiffies + msecs_to_jiffies(100));
		//printk("dectBtn_timer_handler: dectBtn_pressed() == true.\n");
    }
    else {
        atomic_set(&dectBtn_active, 0);
		doPushButtonRelease(PB_BUTTON_4, currentJiffies);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqUnmask(dectBtn_irq);
#else
        BcmHalInterruptEnable(dectBtn_irq);
#endif
		//printk("dectBtn_timer_handler: dectBtn_pressed() == false.\n");
    }
}

void dectBtn_defaultAction(unsigned long time, unsigned long param)
{
    wake_up_interruptible(&g_board_wait_queue);
}
...
static irqreturn_t dectBtn_isr(int irq, void *dev_id)
{
	...
	//printk("dectBtn_isr: triggered(%d).\n", IRQ_HANDLED);	//timliu debug

	if (IRQ_HANDLED == ret) {
        int timerSet = mod_timer(&dectBtn_timer, (currentJiffies + msecs_to_jiffies(100)));	/* 100 msec */
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqMask(irq);
#endif
		if ( 0 == timerSet ) {
            atomic_set(&dectBtn_active, 1);
            doPushButtonPress(PB_BUTTON_4, currentJiffies);
		}
	}
...
static int __init dectBtn_mapIntr(int context)
{
    ...
    if( BpGetDECTExtIntr(&dectBtn_irq) == BP_SUCCESS )
    {
        BpGetDECTExtIntrGpio(&dectBtn_gpio);
        if( dectBtn_irq != BP_EXT_INTR_NONE )
        {
#if defined(CONFIG_BCM960333)
            if( dectBtn_gpio != BP_NOT_DEFINED && dectBtn_irq != BP_EXT_INTR_NONE)
                mapBcm960333GpioToIntr(dectBtn_gpio & BP_GPIO_NUM_MASK, dectBtn_irq);
#endif
            printk("DECT: Button Interrupt 0x%x is enabled\n", dectBtn_irq);
        }
        else
        {
            if( dectBtn_gpio != BP_NOT_DEFINED )
            {
                printk("DECT: Button Polling is enabled on gpio %x\n", dectBtn_gpio);
                kerSysSetGpioDirInput(dectBtn_gpio);
                dectBtn_polling = 1;
            }
        }
    }
    ...
    if( dectBtn_irq != BP_EXT_INTR_NONE )
    {
        ext_irq_idx = (dectBtn_irq&~BP_EXT_INTR_FLAGS_MASK)-BP_EXT_INTR_0;
#if defined(CONFIG_BCM963381) || defined(CONFIG_BCM96848)
        kerSysInitPinmuxInterface(BP_PINMUX_FNTYPE_IRQ | ext_irq_idx);
#endif
        if (!IsExtIntrConflict(extIntrInfo[ext_irq_idx]))
        {
            ...
            if (IsExtIntrShared(dectBtn_irq))
            {
                /* get the gpio and make it input dir */
                if( dectBtn_gpio != BP_NOT_DEFINED )
                {
                    dectBtn_gpio &= BP_GPIO_NUM_MASK;;
                    printk("DECT: Button Interrupt gpio is %d\n", dectBtn_gpio);
                    kerSysSetGpioDirInput(dectBtn_gpio);
                    dev = dectBtn_gpio;
                }
                else
                {
                    printk("DECT: Button Interrupt gpio definition not found \n");
                    hookisr = 0;
                }
            }
            ...
            if(hookisr)
            {
                init_timer(&dectBtn_timer);
                dectBtn_timer.function = dectBtn_timer_handler;
                dectBtn_timer.expires  = jiffies + msecs_to_jiffies(100);
                dectBtn_timer.data     = 0;
                atomic_set(&dectBtn_active, 0);
				atomic_set(&dectBtn_forced, 0);
                dectBtn_irq = map_external_irq (dectBtn_irq);
				ret = 1;
                BcmHalMapInterrupt((FN_HANDLER)dectBtn_isr, (unsigned int)&dev, dectBtn_irq);
#if !defined(CONFIG_ARM)
                BcmHalInterruptEnable(dectBtn_irq);
#endif
            }
...
static void __init dect_board_init()
{
    int ret;
    ret = dectBtn_mapIntr(0);
    if (ret) {
		registerPushButtonPressNotifyHook(PB_BUTTON_4, dectBtn_defaultAction, 0);
	}
}

static void __exit dect_board_deinit()
{
	if(dectBtn_polling == 0 && dectBtn_irq != BP_NOT_DEFINED )
    {
        int ext_irq_idx = dectBtn_irq - INTERRUPT_ID_EXTERNAL_0;
        if(dectBtn_irq) {
            del_timer(&dectBtn_timer);
            atomic_set(&dectBtn_active, 0);
            atomic_set(&dectBtn_forced, 0);
            if (!IsExtIntrShared(extIntrInfo[ext_irq_idx])) {
                BcmHalInterruptDisable(dectBtn_irq);
            }
        }
    }
}
#endif  //defined(ZYXEL_EXTERNAL_DECT) && (ZYXEL_EXTERNAL_DECT == 1)
//20140103, Timliu ---
...
#endif	//#if !defined(ZYXEL) || defined(ZYXEL_KERNEL_RESET_BTN)
...
#define offset(type, elem) ((int)&((type *)0)->elem)
...
#if defined(ZYXEL) && (ZYXEL==1)
#if defined(ZYXEL_SFP_MODULE_SUPPORT)
	struct proc_dir_entry *pSFP;
#endif
#if defined(ZYXEL_ZIGBEE_SUPPORT)
	struct proc_dir_entry *pZIGBEE;
#endif
	struct proc_dir_entry *pZYXEL;
#endif
...
#if defined(ZYXEL) && (ZYXEL==1)
    p1 = create_proc_entry("gpio", 0644, NULL);
    if (p1 == NULL)
        return -1;
...
#if defined(ZYXEL_SFP_MODULE_SUPPORT)
	/* SFP device */
	pSFP = proc_mkdir("sfp", NULL);
	if (pSFP == NULL){
		printk("add_proc_files: failed to create proc files (SFP)!\n");
		return -1;
	}

#if defined(ZYXEL_SFP_DETECT_PIN)
	p1 = create_proc_entry("present", 0644, pSFP);//MOD_ABS
	if (p1 == NULL)
		return -1;
	p1->read_proc  = proc_get_sfp_present;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p1->owner		= THIS_MODULE;
	...
	p2 = create_proc_entry("ae_los", 0644, pSFP);
	if (p2 == NULL)
		return -1;
	p2->read_proc  = proc_get_sfp_ae_los;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p2->owner		= THIS_MODULE;
...
#if defined(ZYXEL_SFP_TX_FAULT_PIN)
	p4 = create_proc_entry("tx_fault", 0644, pSFP);
	if (p4 == NULL)
		return -1;
	p4->read_proc  = proc_get_sfp_tx_fault;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
...
#if defined(ZYXEL_SFP_TX_DISABLE)
	p4 = create_proc_entry("tx_disable", 0644, pSFP);
	if (p4 == NULL)
		return -1;
	p4->read_proc  = proc_get_sfp_tx_disable;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p4->owner		= THIS_MODULE;
...
#endif //end #if defined(ZYXEL_SFP_MODULE_SUPPORT)

#if defined(ZYXEL_ZIGBEE_SUPPORT)
	pZIGBEE = proc_mkdir("zigbee", NULL);
	if (pZIGBEE == NULL){
		printk("add_proc_files: failed to create proc files (Zigbee)!\n");
		return -1;
	}

	p1 = create_proc_entry("enable", 0644, pZIGBEE);  // GPIO state to control power for Zigbee module.
	if (p1 == NULL)
		return -1;
	p1->read_proc  = proc_get_zigbee_enable;
    p1->write_proc  = proc_set_zigbee_enable;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p1->owner		= THIS_MODULE;
	...
	p2 = create_proc_entry("reset", 0644, pZIGBEE);  // GPIO state to reset Zigbee module.
	if (p2 == NULL)
		return -1;
	p2->read_proc  = proc_get_zigbee_reset;
    p2->write_proc  = proc_set_zigbee_reset;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p2->owner		= THIS_MODULE;
	...
	p4 = create_proc_entry("stop_btl", 0644, pZIGBEE);  // GPIO state to stop Zigbee module in bootloader.
	if (p4 == NULL)
		return -1;
	p4->read_proc  = proc_get_zigbee_stop_btl;
    p4->write_proc  = proc_set_zigbee_stop_btl;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	//New linux no longer requires proc_dir_entry->owner field.
#else
	p4->owner		= THIS_MODULE;
...
#endif //end #if defined(ZYXEL_ZIGBEE_SUPPORT)

	/* zyxel directory */
		pZYXEL = proc_mkdir("zyxel", NULL);
		if (pZYXEL == NULL){
			printk("%s,%d: failed to create proc files!\n",__func__,__LINE__);
			return -1;
		}
		/* detect voice board */
		p1 = create_proc_entry("voip_support", 0444, pZYXEL);
		if (p1 == NULL){
			printk("%s,%d: failed to create proc files!\n",__func__,__LINE__);
			return -1;
		}
		p1->read_proc  = proc_get_zyxel_voip_support;
#if defined(ZYXEL_FEATURE_BITS)
        p1 = create_proc_entry("zy_feature_bits", 0444, pZYXEL);
        if (p1 == NULL)
        {
            printk("add_proc_files: failed to create proc files!\n");
            return -1;
        }
        p1->read_proc = proc_get_zyxel_zy_feature_bits;
        p1->write_proc = NULL;
#endif
#if defined(ZYXEL_BOARD_DETECT_BY_GPIO)
        p1 = create_proc_entry("board_detect", 0444, pZYXEL);
        if (p1 == NULL)
        {
            printk("add_proc_files: failed to create proc files!\n");
            return -1;
        }
        p1->read_proc = proc_board_detect_by_gpio;
        p1->write_proc = NULL;
#endif
		...
		p1->owner		= THIS_MODULE;
#endif
#endif //end #if defined(ZYXEL) && (ZYXEL==1)
...
static int del_proc_files(void)
{
	...
	remove_proc_entry("zyxel", NULL);
#if defined(ZYXEL_SFP_MODULE_SUPPORT)
	remove_proc_entry("sfp", NULL);
#endif
#if defined(ZYXEL_ZIGBEE_SUPPORT)
	remove_proc_entry("zigbee", NULL);
#endif
#endif
	remove_proc_entry("wan_type", NULL);
    return 0;
}
...
/**
 * 20130917, TimLiu
 * This function will print out all GPIOs` value.
 * for example: "cat /proc/gpio"
 * */
static int proc_get_gpio(char *page, char **start, off_t off, int cnt, int *eof, void *data)
{
	int r = 0;
	...
	*eof = 1;
	...
	for(i=0;i<GPIO_NUM_MAX;i++) {
		if((i % 6) == 0)
			r += sprintf(page + r, "\n");
		r += sprintf(page + r, "GPIO_%02d= %d\t", i, kerSysGetGpioValue(i));
	}
	r += sprintf(page + r, "\n");
 ...
 * This function expect input in the form of:
 * echo "xxyy" > /proc/gpio
 * where xx is hex for the GPIO number
 * and   yy is hex for the GPIO high(1)/low(0).
 * For example,
 *     echo "0301" > gpio
 * will set high to gpio 3
 */
static int proc_set_gpio(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    char gpiodata[16];
    ...
    str_to_num(input, gpiodata, num_of_octets);
    printk("set GPIO(%d) to %d\n", gpiodata[0], gpiodata[1]);
    kerSysSetGpioState((unsigned short)gpiodata[0], (unsigned short)gpiodata[1]);

    return cnt;
}

/*--------------------------------------
	cat /proc/zyxel/voip_support
	for checking voice board
 *--------------------------------------*/
static int proc_get_zyxel_voip_support(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
    int r = 0;
    NVRAM_DATA *pNvramData;
    ...
    *eof = 1;
    ...
    if (NULL != (pNvramData = readNvramData()))
    {
#if defined(ZYPRJ_VMG9823) || defined(ZYPRJ_VMG4825)
		if( (pNvramData->szVoiceBoardId)==NULL ||
			!strncmp(VOICECFG_NOSLIC_STR, pNvramData->szVoiceBoardId, strlen(VOICECFG_NOSLIC_STR)) || 
			!strcmp("VMG4825-B10A", pNvramData->ProductName) )
#else
		if( (pNvramData->szVoiceBoardId)==NULL ||
			!strncmp(VOICECFG_NOSLIC_STR, pNvramData->szVoiceBoardId, strlen(VOICECFG_NOSLIC_STR)) )
#endif
 ...
 *               kernel command is "$cat /proc/zyxel/zy_feature_bits".
 */
static int proc_get_zyxel_zy_feature_bits(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
    int r = 0;
...
#if defined(ZYXEL_SFP_DETECT_PIN)
/*!
 *  Check SFP module exist or not.
 *
 *  @return     0:not exist; 1:exist; -1:detect GPIO fail.
 */
static int proc_get_sfp_present(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
	int r = 0, ret = BP_SFP_NOT_DEFINE;
    unsigned short isActiveHigh = 0;
    int gpioValue = 0;
	*eof = 1;

    if(BpGetSFPDetectGpio(&bpGpio) == BP_SUCCESS)
    {
        gpioValue = kerSysGetGpioValue(bpGpio);
        isActiveHigh = ((bpGpio & BP_ACTIVE_MASK) == BP_ACTIVE_HIGH);
        if ((gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh))
        {
            ret = BP_SFP_ENABLE;
        }
        else
...
#endif
/*!
 *  AE_LOS/RX_LOS indicate link signals fail or not.
 *
 *  @return     0:ok; 1:fail; -1:detect GPIO fail.
 */
static int proc_get_sfp_ae_los(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
	int r = 0, ret = BP_SFP_NOT_DEFINE;
    unsigned short isActiveHigh = 0;
	...
	*eof = 1;
    ...
    if(BpGetSgmiiGpios(&bpGpio) == BP_SUCCESS)
    {
        gpioValue = kerSysGetGpioValue(bpGpio);
        isActiveHigh = ((bpGpio & BP_ACTIVE_MASK) == BP_ACTIVE_HIGH);
        if ((gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh))
...
#if defined(ZYXEL_SFP_TX_FAULT_PIN)
/*!
 *  TX_FAULT used to check module transmitter has detected a
 *  fault condition related to laser operation or safety.
 *
 *  @return     0:ok; 1:fail; -1:detect GPIO fail.
 */
static int proc_get_sfp_tx_fault(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
	int r = 0, ret = BP_SFP_NOT_DEFINE;
    unsigned short isActiveHigh = 0;
	...
	*eof = 1;
    ...
    if(BpGetSFPTXFaultGpio(&bpGpio) == BP_SUCCESS)
    {
        gpioValue = kerSysGetGpioValue(bpGpio);
        isActiveHigh = ((bpGpio & BP_ACTIVE_MASK) == BP_ACTIVE_HIGH);
        if ((gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh))
...
#if defined(ZYXEL_SFP_TX_DISABLE)
/*!
 *  TX_DISABLE used to turn on/off the transmitter output.
 *
 *  @return     0:Turn on; 1:Turn off; -1:detect GPIO fail.
 */
static int proc_get_sfp_tx_disable(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
	int r = 0, ret = BP_SFP_NOT_DEFINE;
    unsigned short isActiveHigh = 0;
...
#if defined(ZYXEL_ZIGBEE_SUPPORT)
/*!
 *  Used check power on/off state of Zigbee module.
 *
 *  @return     1: Power on; 0:Power off; -1:detect GPIO fail.
 */
static int proc_get_zigbee_enable(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
	int r = 0, ret = BP_ZIGBEE_NOT_DEFINE;
    unsigned short isActiveHigh = 0;
    int gpioValue = 0;
	*eof = 1;
    ...
    if(BpGetZigbeePwrEnGpio(&bpGpio) == BP_SUCCESS)
    {
        ...
        if ((gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh))
        {
            ret = BP_ZIGBEE_ENABLE;
        }
        else
        {
            ret = BP_ZIGBEE_DISABLE;
        }
    }
	r += sprintf(buf, "%d\n", ret);
...
/*!
 *  Used to power on/off the Zigbee module.
 *
 *  @return     1: Power on; 0:Power off; -1:detect GPIO fail.
 */
static int proc_set_zigbee_enable(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    ...
    if (strncasecmp(input, "1", 1) == 0)
    {
        kerSysSetGpioState(bpGpio,kGpioActive);
    }
    else if (strncasecmp(input, "0", 1) == 0)
    {
        kerSysSetGpioState(bpGpio,kGpioInactive);
    }
    else
...
/*!
 *  Used to check the reset state Zigbee module.
 *
 *  @return     1: module reset mode; 0: module normal mode; -1:detect GPIO fail.
 */
static int proc_get_zigbee_reset(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
...
/*!
 *  Used to reset Zigbee module.
 *
 *  @return     1: Trigger module reset; 0: Nothing; -1:detect GPIO fail.
 */
static int proc_set_zigbee_reset(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    char input[2];
...
/*!
 *  Used to check if we want stop Zigbee module in bootloader CLI.
 *
 *  @return     1: Stop in bootloader CLI; 0:Nothing; -1:detect GPIO fail.
 */
static int proc_get_zigbee_stop_btl(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
	unsigned short bpGpio;
...
/*!
 *  Used to stop Zigbee module in bootloader CLI.
 *
 *  @return     1: Stop in bootloader CLI; 0:Nothing; -1:detect GPIO fail.
 */
static int proc_set_zigbee_stop_btl(struct file *f, const char *buf, unsigned long cnt, void *data)
...
static unsigned char _zyBoardDetectByGpio(void)
{
    ...
    for (i = 0; i < ZYXEL_MAX_BOARD_DETECT_GPIO_NUM; i++)
    {
        if (BpGetBoardDetectGpio(&bpGpio, i) == BP_SUCCESS)
        {
            gpioValue = kerSysGetGpioValue(bpGpio);
            if (gpioValue)
            {
        ...
        {
            printk("_zyBoardDetectByGpio: Get GPIO_%d failed, uncorrect board detection.\n", i);
            result_value = 0x1f; //BOARD_DET[4..0]: 11111
        }
    }
    zy_board_detect = result_value;

    return (zy_board_detect);
}
/*!
 *  Get board_detect value.
 *
 *  @note       It's a proc_dir_entry, kernel command is "$cat /proc/zyxel/board_detect".
 */
static int proc_board_detect_by_gpio(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
    int r = 0;
    *eof = 1;
...
#endif //end #if defined(ZYXEL_BOARD_DETECT_BY_GPIO)
...
#if defined(ZYXEL) && (ZYXEL==1) && defined(ZYXEL_FEATURE_BITS)
/*!
 *  Get MRD/NVRAM feature bits (software support or not).
 *
 *  @param[out] *MRDfsStr       MRD feature bits.
 *  @return                     0: success; -1:fail.
 *
 *  @note       8bytes ( MRDfsStr[8..15] )
 */
static int _zyMrdFeatureBitsGet(char *MRDfsStr)
{
    NVRAM_DATA *pNvramData;
    ...
    if (pNvramData == NULL)
    {
        printk("%s_%d: Read nvram data fail!\n", __func__, __LINE__);
        return -1;
    }

    memcpy(MRDfsStr, pNvramData->FeatureBits, NVRAM_FEATUREBITS_LEN);
    kfree(pNvramData);

    return 0;
}
/*!
 *  Check hardware WiFi 2.4G FEM GPIO.
 *
 *  @return     0:without FEM; 1:with FEM; -1:detect GPIO fail.
 *
 *  @note       VMG3925-Bx0C needs to sync with GPIO_17.
 */
static int _zyHwWifi24GFemCheck(void)
{
    unsigned short bpGpio;
    ...
    if (BpGetDetectFEMGpio(&bpGpio) != BP_SUCCESS)
    {
        //printk("%s_%d: Detect GPIO fail!\n", __func__, __LINE__);
        return -1;
    }

    gpioValue = kerSysGetGpioValue(bpGpio);
    isActiveHigh = ((bpGpio & BP_ACTIVE_MASK) == BP_ACTIVE_HIGH);
    //printk("[DEBUG]%s_%d: FEM select GPIO_%d = %d\n", __func__, __LINE__, (bpGpio & BP_GPIO_NUM_MASK), gpioValue);
    if ((gpioValue && isActiveHigh) || (!gpioValue && !isActiveHigh))
    {
        return 1;
    }
...
/*!
 *  Check hardware WiFi 2.4G external antenna.
 *
 *  @return     0:without ext ant; 1:with ext ant -1:detect ext ant fail.
 */
static int _zyHwWifi24GExtAntCheck(void)
{
    return 0;
...
/*!
 *  Check hardware WiFi 5G FEM GPIO.
 *
 *  @return     0:without FEM; 1:with FEM; -1:detect GPIO fail.
 */
static int _zyHwWifi5GFemCheck(void)
...
/*!
 *  Check hardware WiFi 5G external antenna.
 *
 *  @return     0:without ext ant; 1:with ext ant -1:detect ext ant fail.
 */
static int _zyHwWifi5GExtAntCheck(void)
...
/*!
 *  Check if hardware support voice SLIC.
 *
 *  @return     0:without voice SLIC; 1:with voice SLIC; -1:get nvram data fail.
 */
static int _zyHwVoipSlicCheck(void)
{
    NVRAM_DATA *pNvramData;
    ...
    if ((pNvramData->szVoiceBoardId[0] == 0xff) ||
        !strncmp(VOICECFG_NOSLIC_STR, pNvramData->szVoiceBoardId, strlen(VOICECFG_NOSLIC_STR)))
    {
        ret = 0;
    }
    ...
    {
        //printk("[DEBUG]%s_%d: voice boardid = %s.\n", __func__, __LINE__, pNvramData->szVoiceBoardId);
        ret = 1;
    }
    kfree(pNvramData);
...
/*!
 *  Generate and get the final Zyxel Feature Bits of project.
 *
 *  @param[out] zy_feature_bits     A global value for /proc/zyxel/zy_feature_bits.
 *
 *  @note       Read board id string from NVRAM for zyBasedFeatureBitsTbl searching,
 *              also to check/sync with GPIO,NVRAM_FEATURE_BIT,VoIP slic,etc.
 */
static void _zyZyxelFeatureBitsGenerate(void)
{
    char boardid[NVRAM_BOARD_ID_STRING_LEN];
    ZYPROJECT_FEATUREBITS_TABLE *ptr = zyBasedFeatureBitsTbl;
    int i = 0, tmp;
    unsigned int j;

    zy_feature_bits = 0;
    memset(boardid, 0, NVRAM_BOARD_ID_STRING_LEN);
    memset(mrd_feature_bits, 0, NVRAM_FEATUREBITS_LEN);

    kerSysNvRamGetBoardId(boardid);

    /* Get base feature bits first from table */
    while (strcmp(ptr->szBoardId, "NULL") != 0)
    {
        if (!strcmp(ptr->szBoardId, boardid))
        {
            zy_feature_bits = ptr->feature_bits;
            break;
        }
        ptr++;
    }

    if (zy_feature_bits == 0)
    {
        /* No match item in table */
        printk("%s_%d: %s didn't exist in feature bits table.\n", __func__, __LINE__, boardid);
    }
    else
    {
        /* Get MRD feature bits from NVRAM */
        if (!_zyMrdFeatureBitsGet(mrd_feature_bits))
        {
            /* Check/sync ADSL type*/
            // currently do nothing, depends on hardware and based feature bits

            /* Check/sync WiFi FEM GPIO and external antenna */
            if (mrd_feature_bits[MRDFEATUREBITS_BYTE_WIFI] != 0xff)
            {
                tmp = _zyHwWifi24GFemCheck();
                if (tmp == -1)
                {
#if defined(ZYXEL_FEM_DETECT)
                    printk("%s_%d: 2.4G FEM GPIO not exist.\n", __func__, __LINE__);
#endif
                    // If GPIO is non-exist, default should has been ENABLE on based feature bits(Board ID).
                }
                else
                {
                    UPDATE_FEATURE_BITS(tmp, zy_feature_bits, ZYFEATUREBITS_WIFI_24G_FEM);
                }

                tmp = _zyHwWifi24GExtAntCheck();
                if (tmp == -1)
                {
                    printk("%s_%d: 2.4G external antenna not exist.\n", __func__, __LINE__);
                    // If GPIO is non-exist, default should has been ENABLE on based feature bits(Board ID).
                }
                else
                {
                    UPDATE_FEATURE_BITS(tmp, zy_feature_bits, ZYFEATUREBITS_WIFI_24G_EXT_ANT);
                }

                tmp = _zyHwWifi5GFemCheck();
                if (tmp == -1) //detect GPIO fail
                {
                    printk("%s_%d: 5G FEM GPIO not exist.\n", __func__, __LINE__);
                    // If GPIO is non-exist, default should has been ENABLE on based feature bits(Board ID).
                }
                else
                {
                    UPDATE_FEATURE_BITS(tmp, zy_feature_bits, ZYFEATUREBITS_WIFI_5G_FEM);
                }

                tmp = _zyHwWifi5GExtAntCheck();
                if (tmp == -1)
                {
                    printk("%s_%d: 5G external antenna not exist.\n", __func__, __LINE__);
                    // If GPIO is non-exist, default should has been ENABLE on based feature bits(Board ID).
                }
                else
                {
                    UPDATE_FEATURE_BITS(tmp, zy_feature_bits, ZYFEATUREBITS_WIFI_5G_EXT_ANT);
                }
            } // else do nothing

            /* Check/sync with VoIP slic */
            if (mrd_feature_bits[MRDFEATUREBITS_BYTE_VOIP] != 0xff)
            {
                tmp = _zyHwVoipSlicCheck();
                if (tmp == 0) // no slic
                {
                    UPDATE_FEATURE_BITS(0, zy_feature_bits, ZYFEATUREBITS_VOIP);
                }
                else if (tmp == 1) // has slic, software can decide to support or not
                {
                    UPDATE_FEATURE_BITS((mrd_feature_bits[MRDFEATUREBITS_BYTE_VOIP] & MRDFEATUREBITS_VOIP), zy_feature_bits, ZYFEATUREBITS_VOIP);
                }
                else
                {
                    printk("%s_%d: Get voice board id fail!\n", __func__, __LINE__);
                    UPDATE_FEATURE_BITS(0, zy_feature_bits, ZYFEATUREBITS_VOIP); //force disable feature
                }
            } //else do nothing

            /* Check/sync VDSL profile */
            if (mrd_feature_bits[MRDFEATUREBITS_BYTE_VDSLPROFILE] != 0xff)
            {
                for (i = 1, j = 0x1; j <= (MRDFEATUREBITS_VDSL_RESERVERD_LAST); ++i, j = (0x1 << (i - 1)))
                {
                    UPDATE_FEATURE_BITS((mrd_feature_bits[MRDFEATUREBITS_BYTE_VDSLPROFILE] & j), zy_feature_bits, (0x40 << (i - 1)));
                }
            } // else do nothing

            /* Check/sync Zigbee */
            if (mrd_feature_bits[MRDFEATUREBITS_BYTE_ZIGBEE] != 0xff)
            {
                /* ZigBee is software related at the moment */
                UPDATE_FEATURE_BITS((mrd_feature_bits[MRDFEATUREBITS_BYTE_ZIGBEE] & MRDFEATUREBITS_ZIGBEE), zy_feature_bits, ZYFEATUREBITS_ZIGBEE);
            } // else do nothing
        }
        else
        {
            /* Even get fail it's fine, nothing change */
            printk("%s_%d: Get MRD feature bits from NVRAM fail!\n", __func__, __LINE__);
        } // if (!_zyMrdFeatureBitsGet(mrd_feature_bits))
    } // if (zy_feature_bits == 0)
}
/*!
 *  Get WiFi 2.4G with/without FEM.
 *
 *  @return     0:without FEM; 1:with FEM;
 */
int board_zyxelFeatureBitsWifi24GFemGet(void)
{
    return (zy_feature_bits & ZYFEATUREBITS_WIFI_24G_FEM);
}
#endif //#if defined(ZYXEL) && (ZYXEL==1) && defined(ZYXEL_FEATURE_BITS)
...
EXPORT_SYMBOL(BpGetAddlWanPort);
...
EXPORT_SYMBOL(BpGetNTRExternReference);
#endif
...
#if (defined(ZYPRJ_XMG3512)||defined(ZYPRJ_EMG3415)||defined(ZYPRJ_VMG8823_BX0B)||defined(ZYPRJ_XMG3563_B10A)||defined(ZYPRJ_VMG8825_B50B))
EXPORT_SYMBOL(BpGetSelectWanLedGpio);
EXPORT_SYMBOL(BpGetSelectWanLedGpioEth);
#endif
EXPORT_SYMBOL(BpGetSensorEventExtIntr);
EXPORT_SYMBOL(BpGetLanAggregateLinkGpio);
EXPORT_SYMBOL(BpGetLanAggregateActGpio);
#if defined(ZYXEL_BOARD_DETECT_BY_GPIO)
EXPORT_SYMBOL(BpGetBoardDetectGpio);
#endif
#endif
