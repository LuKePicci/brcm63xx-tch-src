...
typedef enum {
    ...
    rdpa_egress_tm_attr_queue_cfg = 9, /* queue_cfg : RWF : aggregate/33[8] tm_queue_cfg(rdpa_tm_queue_cfg_t) : Queue Parameters Configuration */
    ...
    rdpa_egress_tm_attr_queue_stat = 12, /* queue_stat : RWF : aggregate/16[8(tm_queue_index)] rdpa_stat_1way(rdpa_stat_1way_t) : Retrieve Egress Queue Statistics */
    rdpa_egress_tm_attr_queue_occupancy = 13, /* queue_occupancy : RF : number/4[8(tm_queue_index)] : Retrieve Egress Queue Occupancy */
    rdpa_egress_tm_attr_subsidiary = 14, /* subsidiary : RWF : object/4[32] : Next Level Egress-TM */
    rdpa_egress_tm_attr_weight = 15, /* weight : RW : number/4 : Weight for WRR scheduling (0 for unset) */
 ...
 * The function can be called in task context only.
 */
static inline int rdpa_egress_tm_queue_occupancy_get(bdmf_object_handle mo_, rdpa_tm_queue_index_t * ai_, uint32_t *queue_occupancy_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attrelem_get_as_num(mo_, rdpa_egress_tm_attr_queue_occupancy, (bdmf_index)ai_, &_nn_);
    *queue_occupancy_ = (uint32_t)_nn_;
    return _rc_;
}
