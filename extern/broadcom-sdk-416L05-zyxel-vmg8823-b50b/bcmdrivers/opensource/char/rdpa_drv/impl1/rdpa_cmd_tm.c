    ...
    uint32_t *pq_occupancy)
{
    ...
    /* get queue occupancy */
    if ((rc = rdpa_egress_tm_queue_occupancy_get(sched, &q_index, pq_occupancy)))
    {
        CMD_TM_LOG_ERROR("rdpa_egress_tm_queue_occupancy_get() failed: queue_id(%d) ret(%d)", q_index.queue_id, ret);
