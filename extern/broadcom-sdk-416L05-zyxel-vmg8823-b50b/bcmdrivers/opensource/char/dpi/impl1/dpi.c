...
static inline int conntrack_max_dpi_pkt( struct nf_conn *ct, int max_pkt )
{
...
inline void conntrack_set_dpistats_idx( struct sk_buff *skb )
{
    (((struct nf_conn *)skb->nfct)->stats_idx) =
                 dpistats_lookup(&((struct nf_conn *)skb->nfct)->dpi);
}
                                      ...
                                      tdts_pkt_parameter_t *pkt_param )
{
    ...
    if (tdts_check_pkt_parameter_res(pkt_param, TDTS_RES_TYPE_APPID))
    {
...
#endif
        {
            ...
            /*
             * if devid is not set yet, we cannot accelerate the flow.
             */
            if ((conntrack_get_devid(skb) == DEVINFO_IX_INVALID))
            {
#if defined(CONFIG_BCM_KF_BLOG)
                blog_skip(skb);
#endif
        ...
        else if (conntrack_max_dpi_pkt((struct nf_conn*)skb->nfct, dpi_max_pkt))
        {
            conntrack_set_dpi_status(skb, APPID_STATUS_NOMORE);
            dpi_print(" exceed dpi_max_pkt<%d>\n", dpi_max_pkt);
        }
        else
        {
#if defined(CONFIG_BCM_KF_BLOG)
...
#endif
{
    ...
    if (likely(!conntrack_get_dpi_status(skb, APPID_STATUS_NOMORE))||
               blog_request(FLOWTRACK_ALG_HELPER, (void *)skb->nfct, 0, 0))
    {
        dpi_classify(skb);
    }
    ...
    if ((conntrack_get_appid(skb) != 0) &&
        (conntrack_get_devid(skb) != DEVINFO_IX_INVALID))
        conntrack_set_dpistats_idx(skb);
...
static void dpi_nl_process_maxpkt(struct sk_buff *skb)
{
    ...
    int *cfg = (int *)(ptr + sizeof(DpictlMsgHdr_t));
