...
#if !defined(CONFIG_BCM96838) && !defined(CONFIG_BCM96848)
#if defined(ZYXEL_DISABLE_TXRX_FOR_WAN_ACT)
...
int bcmeapi_ioctl_ethsw_get_port_emac(struct ethswctl_data *e)
{
    ...
    if (copy_to_user((void*)(&e->emac_stats_s), (void*)&cached_rdpa_emac_stat[port_cfg.emac], sizeof(struct emac_stats)))
    {
      rc = -EFAULT;
      goto unlock_exit;
    }
...
int bcmeapi_ioctl_ethsw_phy_mode(struct ethswctl_data *e, int phy_id)
{
    if ((phy_id & MAC_IFACE) == MAC_IF_SERDES )
    {
        if (e->type == TYPE_SET)
        {
            ethsw_serdes_set_speed(phy_id, e->speed);
            return BCM_E_NONE;
        }
        else
        {
            e->duplex = 1;
            e->speed = ethsw_serdes_get_speed(phy_id);
        }
    }
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    rdpa_port = rdpa_if_wan0; /* Only wan0 is a valid Runner port for 63138/148 */
#endif
