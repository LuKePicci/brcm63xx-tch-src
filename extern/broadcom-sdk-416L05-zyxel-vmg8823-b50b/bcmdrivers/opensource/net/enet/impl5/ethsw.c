...
/*+++ Kide@zyxel:
+ * switch ports grouping configuration.
+ */
#include <rtk_types.h>
#include <svlan.h>
#include <port.h>
...
extern int (*rtk_switch_ports_grouping_conf_hook)(unsigned int grp_svid, struct net_device *prt, bool add);

void rtk_switch_ports_grouping_conf(unsigned int grp_svid, struct net_device *prt, bool add)
{
    struct net_device *dev_p = prt;
    rtk_svlan_memberCfg_t svlan_cfg;
    rtk_port_t port;

    while (!netdev_path_is_root(dev_p))
        dev_p = netdev_path_next_dev(dev_p);

    if (strcmp(dev_p->name, "eth3") == 0)
        port = UTP_PORT1;
    else if (strcmp(dev_p->name, "eth2") == 0)
        port = UTP_PORT2;
    else if (strcmp(dev_p->name, "eth1") == 0)
        port = UTP_PORT3;
    else if (strcmp(dev_p->name, "eth0") == 0)
        port = UTP_PORT4;
    else
        return -1;

    memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
    rtk_svlan_memberPortEntry_get(grp_svid, &svlan_cfg);

    if (add) {
        // update or create new one
        svlan_cfg.svid = grp_svid;
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, EXT_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, port);
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, port);
        rtk_svlan_memberPortEntry_set(grp_svid, &svlan_cfg);
    } else {
        // delete port or delete the entry
        RTK_PORTMASK_PORT_CLEAR(svlan_cfg.memberport, port);
        RTK_PORTMASK_PORT_CLEAR(svlan_cfg.untagport, port);
        if (!(svlan_cfg.memberport.bits[0] & ~(1 << EXT_PORT1))) {
            memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
            svlan_cfg.svid = grp_svid;
        }
        rtk_svlan_memberPortEntry_set(grp_svid, &svlan_cfg);
...
extern void (*rtk_switch_cpu_port_learning_disable_hook)(bool disable);
void rtk_switch_cpu_port_learning_disable(bool disable)
{
    rtk_l2_flushCfg_t flushcfg;
    rtk_l2_ucastAddr_t l2mac;
    rtk_mac_t   cpu_mac;
    static int curcfg = -1, origcfg = -1;

    if (pVnetDev0_g && pVnetDev0_g->dev)
        memcpy(cpu_mac.octet, pVnetDev0_g->dev->dev_addr, ETHER_ADDR_LEN);

    /* retrieve current setting first */
    rtk_l2_limitLearningCnt_get(EXT_PORT1, &curcfg);

    if (disable && curcfg > 0) {
        if (rtk_l2_addr_get(&cpu_mac, &l2mac) == 0) {
            if (l2mac.port == EXT_PORT1 && l2mac.is_static == 0) {
                l2mac.is_static = 1;
                rtk_l2_addr_add(&cpu_mac, &l2mac);
            } else
                memset(&l2mac, 0, sizeof(rtk_l2_ucastAddr_t));
        }

        rtk_l2_limitLearningCnt_set(EXT_PORT1, 0);
        origcfg = curcfg;

        /* flush l2 entries for cpu port */
        memset(&flushcfg, 0, sizeof(rtk_l2_flushCfg_t));
        flushcfg.flushByPort = ENABLED;
        flushcfg.port = EXT_PORT1;
        flushcfg.flushStaticAddr = 0;
        rtk_l2_ucastAddr_flush(&flushcfg);
    } else if (disable == 0 && curcfg == 0) {
        if (memcmp(l2mac.mac.octet, cpu_mac.octet, ETHER_ADDR_LEN) == 0)
            rtk_l2_addr_del(&cpu_mac, &l2mac);

        rtk_l2_limitLearningCnt_set(EXT_PORT1, (origcfg == -1) ? 2112 : origcfg);
    }
...
void ethsw_init_config(int unit, uint32_t port_map,  int wanPort)
{
    ...
    /*+++ Kide@zyxel: hook function invoked from bridge module */
    rtk_switch_ports_grouping_conf_hook = rtk_switch_ports_grouping_conf;
	rtk_switch_cpu_port_learning_disable_hook = rtk_switch_cpu_port_learning_disable;
