...
#define ETHERNET_PROC_DIR   "ethernet"
#define SFP_STATUS_PROC_NAME    ETHERNET_PROC_DIR "/sfp_status"
#define CROSSBAR_STATUS_PROC_NAME    ETHERNET_PROC_DIR "/crossbar_status"
...
#if defined(CONFIG_4x2_CROSSBAR_SUPPORT) || defined(CONFIG_5x3_CROSSBAR_SUPPORT) || defined(CONFIG_3x2_CROSSBAR_SUPPORT)
static int proc_get_crossbar_status(char *page, char **start, off_t off, int cnt, int *eof, void *data)
{
    ...
    char *intPortName[] = {"Switch Port 7", "WAN Port"};
    char *extPortName[]= {"SerDes", "GPHY4", "xMII"};
...
void ethsw_init_serdes(void)
{
    ...
    proc_mkdir(ETHERNET_PROC_DIR, NULL );
...
/* Patch 5 delete here, but CSP1095113 conflict Patch5.
	ethsw_phy_rreg(phyId, MII_CTRL1000, &mii_gb_ctrl);
	if ((mii_gb_ctrl & ADVERTISE_1000FULL) || (mii_gb_ctrl & ADVERTISE_1000HALF)) { // only GE phy support ethernet@wirespeed
