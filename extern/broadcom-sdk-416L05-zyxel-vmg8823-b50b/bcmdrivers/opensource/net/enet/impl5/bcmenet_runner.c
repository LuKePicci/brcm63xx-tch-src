...
extern uint32 trunk_grp[];
#endif
extern int32 g_addl_wan;
rdpa_if g_lan_as_wan_rdpa_if = rdpa_if_none;
...
static int bcm_enet_rdp_config_bond(rdpa_if rdpa_bond, rdpa_ports rdpa_if_port_map)
{
    bdmf_object_handle port_obj = NULL;
    bdmf_object_handle bond_obj = NULL;
    int rc;
    rdpa_if rdpa_port = rdpa_if_first;
    /* get the rdpa bond port in order to link to lan ports */
     ...
     {
         printk("\n%s() - Error : rdpa bond %d already exist\n\n",__FUNCTION__, rdpa_bond);
         return -1;
     }
     while(rdpa_if_port_map && rdpa_port < rdpa_if_lan_max)
     {
         if ( !(rdpa_if_port_map & rdpa_if_id(rdpa_port)) )
         {
             rdpa_port++;
             continue;
         }
         ...
         if (rc)
         {
             printk("\n%s() - Error : NO rdpa port for rdpa_if %d port_map=0x%llx \n\n",__FUNCTION__, rdpa_port,rdpa_if_port_map);
             return -1;
         }
         ...
         if (rc)
         {
             printk("%s %s Failed to link bond port %d port_map=0x%llx\n", __FILE__, __FUNCTION__, rdpa_bond,rdpa_if_port_map);
             return -1;
         }
         rdpa_if_port_map &= ~(1<<rdpa_port); /* Remove the port from port_map */
         rdpa_port++;
         bdmf_put(port_obj);
     }
     bdmf_put(bond_obj);
...
int bcmenet_create_bonding_ports(void)
{
    rdpa_if rdpa_bond = rdpa_if_bond0;
    rdpa_if rdpa_port;
    rdpa_ports rdpa_if_port_map = 0;

#if defined(CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT)
    {
        int grp_no = 0;
        int logical_port, port_map;
        for (; grp_no < BCM_SW_MAX_TRUNK_GROUPS; grp_no++)
        {
            rdpa_if_port_map = 0;

            if ( trunk_grp[grp_no] ) { /* trunk group in use */
                port_map = trunk_grp[grp_no] & TRUNK_GRP_PORT_MASK;
                while (port_map)
                {
                    logical_port = ffs(port_map)-1;
                    rdpa_port = rdpa_if_lan0+LOGICAL_PORT_TO_PHYSICAL_PORT(logical_port);
                    if (rdpa_port == rdpa_if_lan7)
                        rdpa_port = rdpa_if_lan6;
                    rdpa_if_port_map |= rdpa_if_id(rdpa_port);
                    port_map &= ~(1<<logical_port); /* Remove the port from port_map */
                }

                if (bcm_enet_rdp_config_bond(rdpa_bond, rdpa_if_port_map) )
                {
                    printk("\n%s() - Error : creating bond object for grp_no %d\n\n",__FUNCTION__, grp_no);
                    return -1;
                }
                rdpa_bond++;
            }
    ...
    if ( g_addl_wan != -1 )
    {
        /* Create a bonding group for rdpa_if_wan0 and this lan port */
        rdpa_if_port_map = 0;
        rdpa_if_port_map |= rdpa_if_id(rdpa_if_wan0);
        rdpa_port = rdpa_if_lan0+LOGICAL_PORT_TO_PHYSICAL_PORT(g_addl_wan);
        if (rdpa_port == rdpa_if_lan7)
            rdpa_port = rdpa_if_lan6;
        rdpa_if_port_map |= rdpa_if_id(rdpa_port);
        if (bcm_enet_rdp_config_bond(rdpa_bond, rdpa_if_port_map))
        {
            printk("\n%s() - Error : creating bond object for  port_map 0x%llx\n\n",__FUNCTION__, rdpa_if_port_map);
            return -1;
        }
        /* Store the rdpa_if value for lan */
        g_lan_as_wan_rdpa_if = rdpa_port;
    }
    return 0;
}
...
static void serdes_work_around(int phy_id)
{
    ...
    if (serdesMuxExtPort == -1 || (ethsw_serdes_get_speed(SERDES_PHY_ID) & BMCR_SPEED100) == 0 || 
...
static int bcmenet_create_runner_ring(int ring_id, uint32_t size,uint32_t **ring_base)
{
    ...
    p_ring->buff_cache = (uint32_t *)(CACHED_MALLOC_ATOMIC(sizeof(uint32_t) * MAX_BUFFERS_IN_RING_CACHE));
...
static int _rdpa_cfg_cpu_reason_to_queue_init(void)
{
    ...
    while (!rdpa_cpu_reason_cfg_get_next(rdpa_cpu_obj, &reason_cfg_idx))
    {
        ...
        if (reason_cfg_idx.reason == rdpa_cpu_rx_reason_etype_pppoe_d ||
            ...
            reason_cfg_idx.reason == rdpa_cpu_rx_reason_mcast ||
...
#endif
                    {
#if defined(STAR_FIGHTER2) && defined(ZYXEL_SFP_HIGHER_PRIORITY)
                        // need to add, how to dynamic get SFP cb_port
                        if(cb_port == BP_CROSSBAR_PORT_GPHY4){ // hard coding, SFP cb_port is 0, ether cb_port is 1
                            phys = ethsw_phy_stat(unit, physical_port, BP_CROSSBAR_PORT_AE);
                            if(phys.lnk == 1){
                                link_change_handler(PHYSICAL_PORT_TO_LOGICAL_PORT(physical_port,0), cb_port,
                                    0, phys.spd1000?1000 : (phys.spd100?100 : 10), phys.fdx);
                            }
                        }
#endif
...
#if defined(STAR_FIGHTER2)
#if (defined(ZYXEL_SFP_MODULE_SUPPORT) && defined(ZYXEL_SFP_HIGHER_PRIORITY))
                    {
                        //NOTE: For XMG3512, here only work on ZYPRJ_HARDWARE_VERSION==3 */
						unsigned short bpGpio;
						int value;
                        if (phys.lnk == 1)
                        {
                            if(cb_port == BP_CROSSBAR_PORT_GPHY4)
                                value = kGpioInactive;
                            else //if(cb_port == BP_CROSSBAR_PORT_AE)
                                value = kGpioActive;
                            if(BpGetSelectWanLedGpio(&bpGpio) == BP_SUCCESS)
                            {
                                printk("Output WAN signal to (%s).\n",((cb_port == BP_CROSSBAR_PORT_GPHY4)?"EthWan":"SFP"));
                                kerSysSetGpioState(bpGpio,value);
                            }
#ifndef ZYXEL_WIND_ITALY_CUSTOMIZATION
                            /* Generic: software control WAN LED */
                            if (BpGetSelectWanLedGpioEth(&bpGpio) == BP_SUCCESS)
                            {
                                kerSysSetGpioState(bpGpio,(!value));
                            }
#endif
