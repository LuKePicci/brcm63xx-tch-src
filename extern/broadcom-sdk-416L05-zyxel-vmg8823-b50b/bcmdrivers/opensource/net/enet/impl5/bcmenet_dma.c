...
#define DMA_FMT3(var, val, val2) "    %-40s = " #val":"#val2 "\n", #var, (int)(var), (int)(var)
                ...
                printk(" Note: DMA splitting is on\n");
                printk( DMA_FMT(pktDmaRx->rxOwnership, %d));
#endif
                printk("\n");
...
#if defined(CONFIG_BCM_PKTDMA_RX_SPLITTING)
                printk( DMA_FMT(pktDmaTx->txOwnership, %d));
#endif
                printk("\n");
...
int bcmeapi_ioctl_ethsw_wrrparam(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        if (put_user(total_pkts_per_round, &e->max_pkts_per_iter)) return -EFAULT;
        if (put_user(cur_rxdma_channels, &e->rx_queues)) return -EFAULT;
        if (clear_user((void *)e->weights, sizeof(e->weights))) return -EFAULT;
        if (clear_user((void *)e->weight_pkts, sizeof(e->weights))) return -EFAULT;
        if (copy_to_user((void *)e->weights, weights, sizeof(e->weights[0])*cur_rxdma_channels)) return -EFAULT;
        if (copy_to_user((void *)e->weight_pkts, weight_pkts, sizeof(e->weight_pkts[0])*cur_rxdma_channels)) return -EFAULT;
    } else {
        if (get_user(total_pkts_per_round, &e->max_pkts_per_iter)) return -EFAULT;
        for(i=0; i<cur_rxdma_channels; i++) {
            if (get_user(weights[i], &e->weights[i])) return -EFAULT;
        }
