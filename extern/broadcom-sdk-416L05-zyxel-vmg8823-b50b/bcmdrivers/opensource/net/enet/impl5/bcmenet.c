...
#define UP_DOWN		1<<0
#define SPEED_LOW	1<<1
#define SPEED_HIGH	1<<2
#define HALF_FULL	1<<3

//#define RTK_PORT0_PVID   1
#define RTK_PORT1_PVID   1
#define RTK_PORT2_PVID   2
#define RTK_PORT3_PVID   3
#define RTK_PORT4_PVID   4

static int ethsw_get_rtl_switch_hw_stats(int port,struct net_device_stats *stats);
#endif
...
#ifndef SUPPORT_RTK_SWITCH
static int bcm_mvtags_len(char *ethHdr);
#else
static int bcm_mvtags_len(char *ethHdr,int logical_port);
#endif
static void bcmenet_update_pbvlan_all_bridge(void);

#ifdef SUPPORT_RTK_SWITCH
static int ethsw_get_rtl_switch_hw_stats(int port,struct net_device_stats *stats);
int enet_gmac_log_port_rtl_switch( void );
static inline int IsGmacPortRTLSwitch( int log_port );
#endif

#ifdef ZYXEL_SW_INTERNET_LED_BLINK //__ZyXEL__, David, support SW Internet LED blinking.
int bcmenet_dataforwarded(void);
extern int (*isEthDataTrafficForwarded)(void);
static uint64 lastTxCounter = 0;
#if defined(CONFIG_BCM963138)
int runnerWanLogPortId = 0;
#endif
#endif /*ZYXEL_SW_INTERNET_LED_BLINK*/
...
/* __ZyXEL__, JackYu apply Path 20170525
 * Add Mirroring packet to specific port for eth wan
 */
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
MirrorCfg gMirrorCfg[2];
//extern void MirrorPacket(struct sk_buff *skb, char *intfName, int stripTag, int need_unshare);
extern  void MirrorPacket(char *mirrorIntf, pNBuff_t pNBuf);
...
static int create_vport(void)
{
    ...
    for ( vport_id = 1, logical_port = 0 ; vport_id < vport_cnt + 1; vport_id++, logical_port++, (*map_p) /= 2)
    {
        ...
        if (logical_port>7)
        {
            printk("==debug_%s: logical_port=%d, physical_port=%d, unit=%d\n", __FUNCTION__, logical_port, physical_port, unit);
            phy_id = 5;
            phy_conn = pVnetDev0->EnetInfo[unit].sw.phyconn[4];
            phy_devName = pVnetDev0->EnetInfo[unit].sw.phy_devName[4];
            port_flags = pVnetDev0->EnetInfo[unit].sw.port_flags[4];
        }
#endif
		...
		/* vport	  -> i:  eth3 | eth2 | eth1 | eth0		   */
		printk("vport_id =%d, logical_port=%d, phyID =%x, phy_conn=%x\n",vport_id, logical_port, phy_id, phy_conn);
...
#ifdef SUPPORT_RTK_SWITCH
        if(logical_port > 8 && logical_port <= 12){//rtk swith port is external swith port,just send packet to port 4.
            pDevCtrl->sw_port_id	= 4;
            netdev_path_set_hw_port(dev, 4, BLOG_ENETPHY);
        }else
#endif
...
#else
static inline int bcm_mvtags_len(char *ethHdr,int logical_port)
#endif
{
    ...
    //for rtl switch need remove the tag,but not external ETH WAN port.
    if (logical_port != GPHY_PORT_ID)
#endif
    if (brcm_type == 0x8100 )
    {
        end_offset += BRCM_TAG_TYPE2_LEN;
...
static int ethsw_get_rtl_switch_hw_stats(int port,struct net_device_stats *stats) {
...

    {
        ...
        stats->multicast = port_cntrs.ifInMulticastPkts;
        stats->rx_broadcast_packets = port_cntrs.ifInBroadcastPkts;
        stats->rx_packets = (unsigned long)(port_cntrs.ifInUcastPkts + port_cntrs.ifInMulticastPkts + port_cntrs.ifInBroadcastPkts);

        // Track RX byte count
        stats->rx_bytes = (unsigned long)port_cntrs.ifInOctets;
        ...
        stats->tx_multicast_packets = port_cntrs.ifOutMulticastPkts;
        stats->tx_broadcast_packets = port_cntrs.ifOutBrocastPkts;
        stats->tx_packets = (unsigned long)(port_cntrs.ifOutUcastPkts + port_cntrs.ifOutMulticastPkts + port_cntrs.ifOutBrocastPkts);
        // Track TX byte count
        stats->tx_bytes = (unsigned long)port_cntrs.ifOutOctets;
        // Track TX packet errors
       stats->tx_dropped = 0;//(unsigned long)ctr32;
    }
    return 0;
}
#endif
...
#else
unsigned short bcm_type_trans(struct sk_buff *skb, struct net_device *dev,int logical_port)
#endif
{
    struct ethhdr *eth;
    unsigned char *rawp;
...
#else
    end_offset = bcm_mvtags_len(skb->data,logical_port);
#endif
    if (end_offset)
    {
...
#ifdef SUPPORT_RTK_SWITCH
/*
rtk switch connect to internal switch port4,correct eth mapping to switch port here,DSL-491GNAU-B1BC port reverse,take care for your own project.
*/
		if(strcmp(dev->name,"eth0") == 0)
			ethsw_get_rtl_switch_hw_stats(0,stats);
		else if(strcmp(dev->name,"eth1") == 0)
			ethsw_get_rtl_switch_hw_stats(1,stats);
		else if(strcmp(dev->name,"eth2") == 0)
			ethsw_get_rtl_switch_hw_stats(2,stats);
		else if(strcmp(dev->name,"eth3") == 0)
			ethsw_get_rtl_switch_hw_stats(3,stats);
		else
#endif
...
#if defined(CONFIG_BCM963138)
/* CSP1041547
 * Refer to  ethsw_get_hw_stats() in bcmsw_runner.c or
 *           sf2_bcmsw_dump_mib_ext() in bcmsw.c
 */
	struct net_device_stats *port_stats = &global.net_device_stats_from_hw;

	//port: Runner Physical WAN Port, Internal switch, uint=0
	port = LOGICAL_PORT_TO_PHYSICAL_PORT(runnerWanLogPortId);
	memset(port_stats,0,sizeof(*port_stats));
	ethsw_get_hw_stats(port, port_stats);
	txCounter = (uint64)port_stats->tx_packets;

#else //!defined(CONFIG_BCM963138)
    ...
    for (port = 0; port < EPHY_PORTS; port++)
    {
        ...
        if ((IsWanPort(phyId) || (priv->wanPort & (1 << port))) && !IsInterSwitchPort(0,port))
        //if (IsWanPort(phyId) || (priv->wanPort & (1 << port)))
#else
        if (IsWanPort(phyId))
#endif
        {
#if 0
/* 20161007, TimLiu, Original approach have two issue!
 * 1. For BCM63168 platform with external switch, the priv->wanPort will be 0x18.
 *    So, the for loop will excute two times on port 3(GPHY) and port 4(external switch).
 *    the for loop always keep the tx_packets count of the last port and it is external switch port(lan port)!
 * 2. I don`t know why ethsw_rreg() always return 0 for MIB count of the port 3, but port 4 is OK!
 *    Use ethsw_get_hw_stats() to replace ethsw_rreg().
 */
...
void link_change_handler(int port, int cb_port, int linkstatus, int speed, int duplex)
{
    ...
    int sw_port;
#if defined(ZCFG_USED)
/* __ZyXEL__, Albert, 20150831, When add Interface Group by MAC, LAN PC change plugin LAN port, interface group setting will also change. */
    char temp[32] = "\0";
#endif
    ...
    if ( vport >= 0 || vport < (sizeof(vnet_dev)/sizeof(vnet_dev[0])))
        ...
        pNetDev = NULL;
	if(pNetDev == NULL)printk("===debug(pNetDev == NULL): vport=%d, port=%d\n", vport, port);
#if defined(BCM_ENET_CB_WAN_PORT_LNX_INTF_SUPPORT)
    if (pNetDev) 
    {
    ...
    if ( NULL == pNetDev )
    {
        printk("===debug: return %s, vport=%d, port=%d\n", __FUNCTION__, vport, port);
    ...
    if ((priv->linkState & mask) != linkMask) {
        ...
        printk("===debug_%s:port=%x; vport=%x", __FUNCTION__,port, vport);
...
#if defined(CONFIG_BCM_JUMBO_FRAME) && !defined(CONFIG_BCM_MAX_MTU_SIZE)
    {
        int we_locked_rtnl = FALSE;
            ...
            priv->linkState |= mask;
			//printk("===debug_%s: priv->linkState=%x, mask=%x\n", __FUNCTION__, priv->linkState, mask);
...
#if defined(ZCFG_USED)
			/* __ZyXEL__, Albert, 20150831, When add Interface Group by MAC, LAN PC change plugin LAN port, interface group setting will also change. */
            sprintf(temp, "%s:up", pNetDev->name);
#endif
...
#if defined(ZCFG_USED)
			/* __ZyXEL__, Albert, 20150831, When add Interface Group by MAC, LAN PC change plugin LAN port, interface group setting will also change. */
            sprintf(temp, "%s:down", pNetDev->name);
#endif
            /* notify linux after we have finished setting our internal state */
            if (netif_carrier_ok(pNetDev) != 0)
...
#else
        //if (IsWanPort(phyId)) {
        //when ethWAN phyId is 4 and IsWanPort(4)=0 always return 0, so we cannot go into if statment.
        if ((IsWanPort(phyId) || (priv->wanPort & (1 << port))) && !IsInterSwitchPort(0,port)) {
#endif
            /* set isEthDataTrafficForwarded() per link status */
            if (linkstatus)
            {
#if defined(CONFIG_BCM963138)
            	runnerWanLogPortId = port;
#endif
            	/* EthWAN UP*/
                if (isEthDataTrafficForwarded == NULL)
			    {
				    printk("set isEthDataTrafficForwarded\n");
				    isEthDataTrafficForwarded = bcmenet_dataforwarded;
			    }
            }
		    else
		    {   /* EthWAN DOWN*/
		        if (isEthDataTrafficForwarded == bcmenet_dataforwarded)
			    {
				    printk("set isEthDataTrafficForwarded to NULL\n");
...
#if defined(ZCFG_USED) /* notify monitor task of userspace when link status changed, porting from 412L08, 414, ZyXEL, John */
#if 0
		kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_STATUS_CHANGED,NULL,0);
#else
/* __ZyXEL__, Albert, 20150831, When add Interface Group by MAC, LAN PC change plugin LAN port, interface group setting will also change. */
		kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_STATUS_CHANGED, temp, sizeof(temp));
#endif
#endif
...
#ifdef SUPPORT_RTK_SWITCH
                    if(VPORT_TO_LOGICAL_PORT(i)>8 && VPORT_TO_LOGICAL_PORT(i)<13) {
                        continue;
                    }
#endif
...
#ifdef SUPPORT_RTK_SWITCH
                            if(i == 5){
                                gmac_link_status_changed(GMAC_PORT_ID, phys.lnk, speed, phys.fdx);
                                link_change_handler(GMAC_PORT_ID,BP_CROSSBAR_NOT_DEFINED, phys.lnk,
												speed, phys.fdx);
                                continue;
                            }
#endif
...
#ifdef SUPPORT_RTK_SWITCH
void print_port_stat(int port_id,rtk_stat_port_cntr_t *port_cntrs){

	if(port_id == 7)
		printk((KERN_CRIT "port %x(CPU Port) stat:\n"),port_id);
	else
		printk((KERN_CRIT "port %x(LAN Port) stat:\n"),port_id);

	printk((KERN_CRIT "ifInOctets:%d\n"),(int)port_cntrs->ifInOctets);
	printk((KERN_CRIT "dot3StatsFCSErrors:%d\n"),port_cntrs->dot3StatsFCSErrors);
	printk((KERN_CRIT "dot3StatsSymbolErrors:%d\n"),port_cntrs->dot3StatsSymbolErrors);
	printk((KERN_CRIT "dot3InPauseFrames:%d\n"),port_cntrs->dot3InPauseFrames);
	printk((KERN_CRIT "dot3ControlInUnknownOpcodes:%d\n"),port_cntrs->dot3ControlInUnknownOpcodes);
	printk((KERN_CRIT "etherStatsFragments:%d\n"),port_cntrs->etherStatsFragments);
	printk((KERN_CRIT "etherStatsJabbers:%d\n"),port_cntrs->etherStatsJabbers);
	printk((KERN_CRIT "ifInUcastPkts:%d\n"),port_cntrs->ifInUcastPkts);
	printk((KERN_CRIT "etherStatsDropEvents:%d\n"),port_cntrs->etherStatsDropEvents);
	printk((KERN_CRIT "etherStatsOctets:%d\n"),(int)port_cntrs->etherStatsOctets);
	printk((KERN_CRIT "etherStatsUndersizePkts:%d\n"),port_cntrs->etherStatsUndersizePkts);
	printk((KERN_CRIT "etherStatsOversizePkts:%d\n"),port_cntrs->etherStatsOversizePkts);
	printk((KERN_CRIT "etherStatsPkts64Octets:%d\n"),port_cntrs->etherStatsPkts64Octets);
	printk((KERN_CRIT "etherStatsPkts65to127Octets:%d\n"),port_cntrs->etherStatsPkts65to127Octets);
	printk((KERN_CRIT "etherStatsPkts128to255Octets:%d\n"),port_cntrs->etherStatsPkts128to255Octets);
	printk((KERN_CRIT "etherStatsPkts256to511Octets:%d\n"),port_cntrs->etherStatsPkts256to511Octets);
	printk((KERN_CRIT "etherStatsPkts512to1023Octets:%d\n"),port_cntrs->etherStatsPkts512to1023Octets);
	printk((KERN_CRIT "etherStatsPkts1024toMaxOctets:%d\n"),port_cntrs->etherStatsPkts1024toMaxOctets);
	printk((KERN_CRIT "etherStatsMcastPkts:%d\n"),port_cntrs->etherStatsMcastPkts);
	printk((KERN_CRIT "etherStatsBcastPkts:%d\n"),port_cntrs->etherStatsBcastPkts);
	printk((KERN_CRIT "ifOutOctets:%d\n"),(int)port_cntrs->ifOutOctets);
	printk((KERN_CRIT "dot3StatsSingleCollisionFrames:%d\n"),port_cntrs->dot3StatsSingleCollisionFrames);
	printk((KERN_CRIT "dot3StatsMultipleCollisionFrames:%d\n"),port_cntrs->dot3StatsMultipleCollisionFrames);
	printk((KERN_CRIT "dot3StatsDeferredTransmissions:%d\n"),port_cntrs->dot3StatsDeferredTransmissions);
	printk((KERN_CRIT "dot3StatsLateCollisions:%d\n"),port_cntrs->dot3StatsLateCollisions);
	printk((KERN_CRIT "etherStatsCollisions:%d\n"),port_cntrs->etherStatsCollisions);
	printk((KERN_CRIT "dot3StatsExcessiveCollisions:%d\n"),port_cntrs->dot3StatsExcessiveCollisions);
	printk((KERN_CRIT "dot3OutPauseFrames:%d\n"),port_cntrs->dot3OutPauseFrames);
	printk((KERN_CRIT "dot1dBasePortDelayExceededDiscards:%d\n"),port_cntrs->dot1dBasePortDelayExceededDiscards);
	printk((KERN_CRIT "dot1dTpPortInDiscards:%d\n"),port_cntrs->dot1dTpPortInDiscards);
	printk((KERN_CRIT "ifOutUcastPkts:%d\n"),port_cntrs->ifOutUcastPkts);
	printk((KERN_CRIT "ifOutMulticastPkts:%d\n"),port_cntrs->ifOutMulticastPkts);
	printk((KERN_CRIT "ifOutBrocastPkts:%d\n"),port_cntrs->ifOutBrocastPkts);
	printk((KERN_CRIT "outOampduPkts:%d\n"),port_cntrs->outOampduPkts);
	printk((KERN_CRIT "inOampduPkts:%d\n"),port_cntrs->inOampduPkts);
	printk((KERN_CRIT "pktgenPkts:%d\n"),port_cntrs->pktgenPkts);
	printk((KERN_CRIT "inMldChecksumError:%d\n"),port_cntrs->inMldChecksumError);
	printk((KERN_CRIT "inIgmpChecksumError:%d\n"),port_cntrs->inIgmpChecksumError);
	printk((KERN_CRIT "inMldSpecificQuery:%d\n"),port_cntrs->inMldSpecificQuery);
	printk((KERN_CRIT "inMldGeneralQuery:%d\n"),port_cntrs->inMldGeneralQuery);
	printk((KERN_CRIT "inIgmpSpecificQuery:%d\n"),port_cntrs->inIgmpSpecificQuery);
	printk((KERN_CRIT "inIgmpGeneralQuery:%d\n"),port_cntrs->inIgmpGeneralQuery);
	printk((KERN_CRIT "inMldLeaves:%d\n"),port_cntrs->inMldLeaves);
	printk((KERN_CRIT "inIgmpLeaves:%d\n"),port_cntrs->inIgmpLeaves);
	printk((KERN_CRIT "inIgmpJoinsSuccess:%d\n"),port_cntrs->inIgmpJoinsSuccess);
	printk((KERN_CRIT "inIgmpJoinsFail:%d\n"),port_cntrs->inIgmpJoinsFail);
	printk((KERN_CRIT "inMldJoinsSuccess:%d\n"),port_cntrs->inMldJoinsSuccess);
	printk((KERN_CRIT "inMldJoinsFail:%d\n"),port_cntrs->inMldJoinsFail);
	printk((KERN_CRIT "inReportSuppressionDrop:%d\n"),port_cntrs->inReportSuppressionDrop);
	printk((KERN_CRIT "inLeaveSuppressionDrop:%d\n"),port_cntrs->inLeaveSuppressionDrop);
	printk((KERN_CRIT "outIgmpReports:%d\n"),port_cntrs->outIgmpReports);
	printk((KERN_CRIT "outIgmpLeaves:%d\n"),port_cntrs->outIgmpLeaves);
	printk((KERN_CRIT "outIgmpGeneralQuery:%d\n"),port_cntrs->outIgmpGeneralQuery);
	printk((KERN_CRIT "outIgmpSpecificQuery:%d\n"),port_cntrs->outIgmpSpecificQuery);
	printk((KERN_CRIT "outMldReports:%d\n"),port_cntrs->outMldReports);
	printk((KERN_CRIT "outMldLeaves:%d\n"),port_cntrs->outMldLeaves);
	printk((KERN_CRIT "outMldGeneralQuery:%d\n"),port_cntrs->outMldGeneralQuery);
	printk((KERN_CRIT "outMldSpecificQuery:%d\n"),port_cntrs->outMldSpecificQuery);
	printk((KERN_CRIT "inKnownMulticastPkts:%d\n"),port_cntrs->inKnownMulticastPkts);
	printk((KERN_CRIT "ifInMulticastPkts:%d\n"),port_cntrs->ifInMulticastPkts);
	printk((KERN_CRIT "ifInBroadcastPkts:%d\n"),port_cntrs->ifInBroadcastPkts);
}

static void rtk_print_snooping_entry(rtk_uint32 address,rtk_l2_mcastAddr_t *l2McastAddr){
	printk((KERN_CRIT "%x		%02x:%02x:%02x:%02x:%02x:%02x		%d/%d		0x%x\n"),
						(unsigned int)address,(unsigned int)l2McastAddr->mac.octet[0],(unsigned int)l2McastAddr->mac.octet[1],(unsigned int)l2McastAddr->mac.octet[2],(unsigned int)l2McastAddr->mac.octet[3],(unsigned int)l2McastAddr->mac.octet[4],(unsigned int)l2McastAddr->mac.octet[5],
						*(unsigned int *)&(l2McastAddr->ivl),(*(unsigned int *)&(l2McastAddr->ivl) ? *(unsigned int *)&(l2McastAddr->vid) : *(unsigned int *)&(l2McastAddr->fid)), *(unsigned int *)&(l2McastAddr->portmask));
}

static void rtk_dump_snooping_entry(int del,int protocol){
	rtk_uint32 address = 0;
    rtk_l2_mcastAddr_t l2mac;
	rtk_api_ret_t ret = 0;

	if(!del){
		printk((KERN_CRIT "Dump snooping entry(Realtek Switch)\n"));
		printk((KERN_CRIT "Address		MAC				ivl/cvid_fid	portmask\n"));
	}

	while(1){
		if((ret = rtk_l2_mcastAddr_next_get(&address,&l2mac)) != 0){
			break;
		}else{
            if(del == 1){
				if(protocol == 1){//igmp
					if(l2mac.mac.octet[0] != 0x01 || l2mac.mac.octet[1] != 0x00 || l2mac.mac.octet[2] != 0x5E){
						address++;
						continue;
					}
				}else if(protocol == 2){//mld
					if(l2mac.mac.octet[0] != 0x33 || l2mac.mac.octet[1] != 0x33){
						address++;
						continue;
					}
				}
				rtk_l2_mcastAddr_del(&l2mac);
			}else{
				rtk_print_snooping_entry(address,&l2mac);
			}
			address++;
		}
	}
...
static void rtk_enable_snooping(void);
static void rtk_enable_snooping(){
	rtk_l2_init();
	rtk_l2_ipMcastAddrLookup_set(LOOKUP_MAC);
}

static void rtk_disable_snooping(int protocol);
static void rtk_disable_snooping(int protocol){
	rtk_dump_snooping_entry(1,protocol);/*flush all snooping entry*/
}

static void rtk_add_snooping_entry(rtk_l2_mcastAddr_t *l2McastAddr){
	rtk_l2_mcastAddr_t l2;

    memset(&l2, 0, sizeof(l2));
    memcpy(&l2, l2McastAddr, sizeof(rtk_l2_mcastAddr_t));
	rtk_l2_mcastAddr_get(&l2);
	l2McastAddr->portmask.bits[0] |= l2.portmask.bits[0];
	rtk_l2_mcastAddr_add(l2McastAddr);
}

static void rtk_del_snooping_entry(rtk_l2_mcastAddr_t *l2McastAddr){
    rtk_l2_mcastAddr_t l2;
    memset(&l2, 0, sizeof(l2));
    memcpy(&l2, l2McastAddr, sizeof(rtk_l2_mcastAddr_t));
	rtk_l2_mcastAddr_get(&l2);
	l2.portmask.bits[0] &= ~(l2McastAddr->portmask.bits[0]);
	if(l2.portmask.bits[0] & (0xFFFF&(~(1 << EXT_PORT1)))){	/*ignore the RTK Switch CPU port*/
		rtk_l2_mcastAddr_add(&l2);
	}else{
		rtk_l2_mcastAddr_del(&l2);
	}
}
#endif
 ...
 */
       ...
       if(NULL == skb->end)
       	{
       	          //printk("skb->end is NULL in xmit\n");
       	          kfree_skb(skb);
			   return NULL;
       	}
 ...
 */
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    if (pParam->blog_phy == BLOG_ENETPHY && pParam->port_id ==wan_port_id && gMirrorCfg[MIRROR_DIR_OUT].nStatus)
    {
        if (pParam->pNBuff != (pNBuff_t)NULL)
        {
            //ENET_TX_UNLOCK();
            MirrorPacket(gMirrorCfg[MIRROR_DIR_OUT].szMirrorInterface,pParam->pNBuff);
            //ENET_TX_LOCK();
        }
    }
#endif
...
#ifdef SUPPORT_RTK_SWITCH
                    if(phy_port_id > 8)
                        blog_chnl = 4;/* blog rx channel is switch port */
                    else
#endif
                    blog_chnl = phy_port_id;/* blog rx channel is switch port */
                    blog_phy = BLOG_ENETPHY;/* blog rx phy type is ethernet */
#if defined(CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT)
                    /* For bonded ports - always pass the same port id (blog_channel) to flow-cache
                       otherwise flow-cache will treat them as different flows. */
                    {
                        int grp_no = 0;
                        for (; grp_no < BCM_SW_MAX_TRUNK_GROUPS; grp_no++)
                        {
                            if ( (trunk_grp[grp_no] & TRUNK_GRP_PORT_MASK) & (1<<phy_port_id) ) { /* Port is part of trunk group_0 */
                                blog_chnl = (trunk_grp[grp_no] & TRUNK_GRP_MASTER_PORT_MASK) >> TRUNK_GRP_MASTER_PORT_SHIFT;
                                break;
                            }
                        }
                    }
 ...
 */
#if defined(CONFIG_BCM963138) ||defined(CONFIG_BCM963148)
        if(blog_phy == BLOG_ENETPHY  && phy_port_id == wan_port_id && gMirrorCfg[MIRROR_DIR_IN].nStatus)
        {
             pNBuff_t pNBuf = FKBUFF_2_PNBUFF(pFkb);

              MirrorPacket(gMirrorCfg[MIRROR_DIR_IN].szMirrorInterface, pNBuf);
...
#else
        skb->protocol = bcm_type_trans(skb, dev,phy_port_id);
#endif
        skb->dev = dev;
        if (global.dump_enable) {
            DUMP_PKT(skb->data, skb->len);
...
static void bcmenet_check_addl_wan_config(void)
{
    ...
    for (port = 0; port_map && (port < MAX_SWITCH_PORTS); port++) /* go through all the switch ports */
    {
        /* NOP if switch port is not part of map */
        if (!(port_map & (1<<port) )) continue;
        /* Remove switch port from port map */
        port_map &= ~(1<<port);
        port_flags = info->sw.port_flags[port];
        if ( IsPortAddlWan(port_flags) )
        {
            ...
            break; /* No need to check further; There cannot be more than one additional WAN */
        }
    }
}
...
int __init bcm63xx_enet_probe(void)
{
	...
	uint32 *regvalue;
	//int ret;
	//	rtk_portmask_t			member, untag;	//unused
...
#ifdef SUPPORT_RTK_SWITCH
      /*Because isolate port by SVLAN tag, it need strip function to know offset of this tag.*/
     /*Reset port map for RTL switch.  Port 0-7 is for internal switch and port 8-15 is for RTL switch.  For this case,
         physical Port 9-12 is RTL swithc port 1-4.*/
     /*Remove Port 4 because it is connected to RTL switch.*/
     pDevCtrl->allPortMap = (EnetInfo[0].sw.port_map & (~(1<<4)));
     pDevCtrl->allPortMap |= 0x1e<<8;
#endif
    ...
    if (bcmenet_create_bonding_ports()) return -1;
...
/* XXX: Move to system init / add ioctl */
...
#ifdef SUPPORT_RTK_SWITCH
    printk("== Init rtk_switch with API V1.3.2.\n");
    ret = rtl8367c_setAsicReg(RTL8367C_REG_CHIP_RESET, RTL8367C_CHIP_RST_MASK);
    printk("rtl8370_setAsicReg call:ret == 0x%08x\n",ret);
    //smi_reset(0,12);
    mdelay(1000);
    /* Chip initialization */
    ret = rtk_switch_init();
    printk("rtk_switch_init ret == 0x%08x\n",ret);
    {
        rtk_uint32 rtl_chipid;
        rtk_uint32 rtl_chipver;

        rtl8367c_setAsicReg(0x13C2, 0x0249);

        rtl8367c_getAsicReg(RTL8367C_REG_CHIP_NUMBER, &rtl_chipid);
        rtl8367c_getAsicReg(RTL8367C_REG_CHIP_VER, &rtl_chipver);
        rtl8367c_setAsicReg(0x13C2, 0x0000);

        printk("RTL: chip %x.%x probed\n", rtl_chipid, rtl_chipver);
        if(rtl_chipid == 0x6367){
            printk("RTL: chip RTL8367RB-VB\n");
        }
    }
    mac_cfg.forcemode = 1;// MAC_FORCE;
    mac_cfg.speed	  = 2; //SPD_1000M;
    mac_cfg.duplex	  = 1;//FULL_DUPLEX;
    mac_cfg.link	  = 1;
    mac_cfg.nway	  = 0;
    mac_cfg.rxpause   = 1;
    mac_cfg.txpause   = 1;
    //ret = rtk_port_macForceLinkExt0_set(1, &mac_cfg);
    ret = rtk_port_macForceLinkExt_set(EXT_PORT1, MODE_EXT_RGMII, &mac_cfg);
    //printk("rtk_port_macForceLinkExt0_set call:ret == 0x%08x\n",ret);
    ...
    ret = rtk_led_enable_set(LED_GROUP_0, &port_mask);/* Turn on LED group 0  at Port 0~ port 4 */
    //printk("rtk_led_enable_set LED_GROUP_0 call:ret == 0x%08x\n",ret);
    ...
    RTK_PORTMASK_PORT_SET(portmask, UTP_PORT3);
    RTK_PORTMASK_PORT_SET(portmask, UTP_PORT4);
    rtk_led_enable_set(LED_GROUP_1, &port_mask);/* Turn on LED group 1  at Port 0~ port 4 */
    //rtl8367b_setAsicReg(0x1306, 0x000a);//rx-tx delay from 0x000f--->0x000d
    //rtl8367b_setAsicReg(RTL8367B_REG_EXT0_RGMXF, 0x000b);//rx-tx delay from 0x000f--->0x000d for 68 ohm
    //rtl8367b_setAsicReg(0x1307, 0x000c);
    //rtl8367b_setAsicReg(RTL8367B_REG_EXT1_RGMXF, 0x000b);//rx-tx delay from 0x000f--->0x000d  for 68 ohm
    {
        rtk_data_t txDelay = 1;
        rtk_data_t rxDelay = 1;

        ret = rtk_port_rgmiiDelayExt_set(EXT_PORT1, txDelay, rxDelay);
        printk("set txDelay %d rxDelay %d, return 0x%x\n",txDelay,rxDelay, ret);
    }

    {
        ...
        rtk_svlan_init();
        rtk_svlan_servicePort_add(EXT_PORT1);
        rtk_svlan_tpidEntry_set((rtk_svlan_tpid_t)0x8100);
        /*		svid		member
         vlan1	1		1000 0010(0x82)
         vlan2	2		1000 0100(0x84)
         vlan3	3		1000 1000(0x88)
         vlan4	4		1001 0000(0x90)
        */
        //	memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
        //	svlan_cfg.svid = RTK_PORT0_PVID;
        //	svlan_cfg.memberport = (1<<RTK_PORT0) | (1<<RTK_PORT_EXT2);
        //	svlan_cfg.untagport = (1<<RTK_PORT0) | (1<<RTK_PORT1) | (1<<RTK_PORT2) | (1<<RTK_PORT3) | (1<<RTK_PORT4);
        //	rtk_svlan_memberPortEntry_adv_set(RTK_PORT0_PVID, &svlan_cfg);

        memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
        svlan_cfg.svid = RTK_PORT1_PVID;
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, UTP_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, EXT_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, UTP_PORT1);
        //rtk_svlan_memberPortEntry_adv_set(RTK_PORT1_PVID, &svlan_cfg);
        rtk_svlan_memberPortEntry_set(RTK_PORT1_PVID, &svlan_cfg);

        memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
        svlan_cfg.svid = RTK_PORT2_PVID;
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, UTP_PORT2);
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, EXT_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, UTP_PORT2);
        //rtk_svlan_memberPortEntry_adv_set(RTK_PORT2_PVID, &svlan_cfg);
        rtk_svlan_memberPortEntry_set(RTK_PORT2_PVID, &svlan_cfg);

        memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
        svlan_cfg.svid = RTK_PORT3_PVID;
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, UTP_PORT3);
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, EXT_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, UTP_PORT3);
        //rtk_svlan_memberPortEntry_adv_set(RTK_PORT3_PVID, &svlan_cfg);
        rtk_svlan_memberPortEntry_set(RTK_PORT3_PVID, &svlan_cfg);

        memset(&svlan_cfg,0,sizeof(rtk_svlan_memberCfg_t));
        svlan_cfg.svid = RTK_PORT4_PVID;
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, UTP_PORT4);
        RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, EXT_PORT1);
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, UTP_PORT4);
        //rtk_svlan_memberPortEntry_adv_set(RTK_PORT4_PVID, &svlan_cfg);
        rtk_svlan_memberPortEntry_set(RTK_PORT4_PVID, &svlan_cfg);

        //rtk_svlan_defaultSvlan_set(RTK_PORT0, RTK_PORT0_PVID);
        rtk_svlan_defaultSvlan_set(UTP_PORT1, RTK_PORT1_PVID);
        rtk_svlan_defaultSvlan_set(UTP_PORT2, RTK_PORT2_PVID);
        rtk_svlan_defaultSvlan_set(UTP_PORT3, RTK_PORT3_PVID);
        rtk_svlan_defaultSvlan_set(UTP_PORT4, RTK_PORT4_PVID);
        ...
        RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, UTP_PORT4);
        //rtk_svlan_memberPortEntry_adv_set(RTK_PORT4_PVID, &svlan_cfg);
        rtk_svlan_memberPortEntry_set(100, &svlan_cfg);
        rtk_svlan_untag_action_set(UNTAG_ASSIGN,100);
    }
#endif
 ...
 */
    ...
    rtk_stat_port_cntr_t port_cntrs;
    int port_index;
    rtk_port_mac_ability_t mAbility;
    rtk_port_phy_ability_t phyAbility;
    static int rtk_igmp_snoop_enable =0;
    static int rtk_mld_snoop_enable = 0;
    ...
    switch (cmd)
    {
        ...
        case SIOCMIBINFO:
#ifdef SUPPORT_RTK_SWITCH
            //rtk swith port is external swith port,just send packet to port 4,it,swPort is always 4, then LOGICAL_PORT_TO_VPORT is -1.
            //	for (vport = 1; vport <= vport_cnt; vport++) {
            //if ((vnet_dev[vport]) &&
            //(strcmp(enetif_data->ifname, vnet_dev[vport]->name) == 0))
            {
                // Create MIB address.
                IOCTL_MIB_INFO *mib;
                mib = &((BcmEnet_devctrl *)netdev_priv(dev))->MibInfo;
                // Copy MIB to caller.
 ...
 */
        ...
        case SIOCPORTMIRROR:
            ...
            else
            {
                ...
                {
                    memcpy(&gMirrorCfg[0], &mirrorCfg, sizeof(MirrorCfg));
                }
                else /* MIRROR_DIR_OUT */
                {
                    memcpy(&gMirrorCfg[1], &mirrorCfg, sizeof(MirrorCfg));
                }
            }
            break;
#endif
		...
		val = kerSysGetMacAddress(dev->dev_addr, kerSysGetMacAddressType(dev->name));
#else
...
#if !defined(CONFIG_BCM96838) && !defined(CONFIG_BCM96848)
#if defined(ZCFG_USED)
				// Need check hw-switching status here. 
				// if hw-switching is disable , don't set port with HW_SWITCH flags
				if(ethsw_get_hw_switching_state() == HW_SWITCHING_ENABLED)
#endif				
				dev->priv_flags |= IFF_HW_SWITCH;
#endif
            }
        ...
        case SIOCETHSWCTLOPS:
            switch(e->op) {
...
#if defined(ZCFG_USED)
                case ETHSWWANDATAFUNC:
                    BCM_ENET_DEBUG("ethswctl ETHSWWANDATAFUNC ioctl");
#ifdef ZYXEL_SW_INTERNET_LED_BLINK //__ZyXEL__, David, support SW Internet LED blinking.
            		if (e->val == 1)
            		{   /* EthWAN UP*/
                		if (isEthDataTrafficForwarded == NULL)
                ...
                case ETHSWUSEDEFTXQ:
                    ...
                    return bcmeapi_ioctl_use_default_txq_config(pEthCtrl,e);
                    break;

                case ETHSWDEFTXQ:
                    BCM_ENET_DEBUG("ethswctl ETHSWDEFTXQ ioctl");
                    pEthCtrl = (BcmEnet_devctrl *)netdev_priv(dev);
                    return bcmeapi_ioctl_default_txq_config(pEthCtrl, e);
                    break;
...
#ifdef SUPPORT_RTK_SWITCH
                    printk("Realtek switch didn`t support this command!\n");
#else
        ...
        case SIOCETHCTLOPS:
            switch(ethctl->op) {
                ...
                case ETHSETSPOWERUP:
                    swPort = port_id_from_dev(dev);
#ifdef SUPPORT_RTK_SWITCH
                    if ((ethctl->phy_addr>=0) && (ethctl->phy_addr<=3)) {
                        val = rtk_port_adminEnable_set(ethctl->phy_addr+1,1);
                    } else if (ethctl->phy_addr == 4) {
                        ethsw_switch_manage_port_power_mode(ethctl->phy_addr-1, 1);
                        val = 0;
                    } else {
...
#ifdef SUPPORT_RTK_SWITCH
                    if ((ethctl->phy_addr>=0) && (ethctl->phy_addr<=3)) {
                        val = rtk_port_adminEnable_set(ethctl->phy_addr+1,0);
                    } else if (ethctl->phy_addr == 4) {
                        ethsw_switch_manage_port_power_mode(ethctl->phy_addr-1, 0);
                        val = 0;
                    } else {
...
#ifdef SUPPORT_RTK_SWITCH
                case RTLLEDON:
                    val =rtk_led_groupConfig_set(LED_GROUP_0, LED_CONFIG_LINK_ACT);
                    val =rtk_led_groupConfig_set(LED_GROUP_1, LED_CONFIG_LINK_ACT);
                    break;

                case RTLLEDOFF:
                    val =rtk_led_groupConfig_set(LED_GROUP_0, LED_CONFIG_LEDOFF);
                    val =rtk_led_groupConfig_set(LED_GROUP_1, LED_CONFIG_LEDOFF);
                    break;
#endif
...
#ifdef SUPPORT_RTK_SWITCH
                case RTLSETMIIREG:
                    BCM_ENET_DEBUG("phy_id: %d; testmode = %d \n", ethctl->phy_addr,
                               ethctl->testmode);
                    val = rtk_port_phyTestMode_set(ethctl->phy_addr,ethctl->testmode);
                    break;
                case RTLGETPORTSTAT:
                    if(ethctl->phy_addr == 8){
                        BCM_ENET_DEBUG("Dump all ports stat: \n");
                        for(port_index=0;port_index<8;port_index++){
                            if(port_index != 5 && port_index != 6 && port_index != 0){
                                val = rtk_stat_port_getAll(port_index,&port_cntrs);
                                print_port_stat(port_index,&port_cntrs);
                            }
                        }
                    }else{
                        BCM_ENET_DEBUG("phy_id: %d \n", ethctl->phy_addr);
                        val = rtk_stat_port_getAll(ethctl->phy_addr,&port_cntrs);
                        print_port_stat(ethctl->phy_addr,&port_cntrs);
                    }
                    break;
                case RTLGETPHYTSTAT:
                    BCM_ENET_DEBUG("phy_id: %d \n", ethctl->phy_addr);
                    val = rtk_port_macStatus_get(ethctl->phy_addr, &mAbility);
                    ethctl->ret_val =0;
                    if(0 == mAbility.speed){
                        ethctl->ret_val &= ~(SPEED_LOW);
                        ethctl->ret_val &= ~(SPEED_HIGH);
                    }else if(1 == mAbility.speed){
                        ethctl->ret_val |= SPEED_LOW;
                    }else if(2 == mAbility.speed){
                        ethctl->ret_val |= SPEED_HIGH;
                    }
                    if(1 == mAbility.duplex){
                        ethctl->ret_val |= HALF_FULL;
                    }else{
                        ethctl->ret_val &= ~(HALF_FULL);
                    }
                    if(1 == mAbility.link){
                        ethctl->ret_val |= UP_DOWN;
                    }else{
                        ethctl->ret_val &= ~(UP_DOWN);
                    }
                    break;
                case RTLSETPHYMEDIATYPE:
                    BCM_ENET_DEBUG("phy_id: %d \n", ethctl->phy_addr);
                    BCM_ENET_DEBUG("\r\n set phyAbility\r\n");
                    val = rtk_port_phyAutoNegoAbility_get(ethctl->phy_addr, &phyAbility);
                    printk("<0>Get AutoNegotiation %d Full_1000 %d Full_100 %d Half_100 %d Full_10 %d Half_10%d FC %d AsyFC %d\r\n",
                    phyAbility.AutoNegotiation,phyAbility.Full_1000,phyAbility.Full_100,phyAbility.Half_100,phyAbility.Full_10,
                    phyAbility.Half_10 ,phyAbility.FC, phyAbility.AsyFC);
                    if(ethctl->val== 0)
                    {
                    ...
                    else if(ethctl->val == 1)
                    {
                        phyAbility.AutoNegotiation = 0;
                        phyAbility.Full_1000 =  0;
                        phyAbility.Full_100 = 1;
                        phyAbility.Half_100= 0;
                        phyAbility.Full_10= 0;
                        phyAbility.Half_10= 0;
                    }
                    else if(ethctl->val == 2)
                    {
                        phyAbility.AutoNegotiation = 0;
                        phyAbility.Full_1000 =  0;
                        phyAbility.Full_100 = 0;
                        phyAbility.Half_100= 1;
                        phyAbility.Full_10= 0;
                        phyAbility.Half_10= 0;
                    }
                    else if(ethctl->val == 3)
                    {
                        phyAbility.AutoNegotiation = 0;
                        phyAbility.Full_1000 =  0;
                        phyAbility.Full_100 = 0;
                    ...
                    val = rtk_port_phyAutoNegoAbility_set(ethctl->phy_addr, &phyAbility);
                    break;
                case RTLENABLESNOOP:
                    if (ethctl->val == 1)
                        rtk_igmp_snoop_enable = 1;
                    if (ethctl->val == 2)
                        rtk_mld_snoop_enable = 1;
                    rtk_enable_snooping();
                    break;
                case RTLDISABLESNOOP:
                    if (ethctl->val == 1)
                        rtk_igmp_snoop_enable = 0;
                    if (ethctl->val == 2)
                        rtk_mld_snoop_enable = 0;
                    rtk_disable_snooping(ethctl->val);
                    break;
                case RTLADDSNOOP:
                    memset(&l2McastAddr,0,sizeof(l2McastAddr));
                    RTK_PORTMASK_PORT_SET(l2McastAddr.portmask, ethctl->phy_addr);
                    RTK_PORTMASK_PORT_SET(l2McastAddr.portmask, EXT_PORT1);
                    l2McastAddr.ivl = 0;
                    l2McastAddr.fid = 0;
                    if(ethctl->flags==1 && rtk_igmp_snoop_enable==1){//IGMP
                        l2McastAddr.mac.octet[0] = 0x01;
                        l2McastAddr.mac.octet[1] = 0x00;
                        l2McastAddr.mac.octet[2] = 0x5E;
                        l2McastAddr.mac.octet[3] = ethctl->val & 0xFF;
                        l2McastAddr.mac.octet[4] = (ethctl->val2 >> 8) & 0xFF;
                        l2McastAddr.mac.octet[5] = ethctl->val2 & 0xFF;
                        rtk_add_snooping_entry(&l2McastAddr);
                    }else if (ethctl->flags==2 && rtk_mld_snoop_enable==1){//MLD
                        l2McastAddr.mac.octet[0] = 0x33;
                        l2McastAddr.mac.octet[1] = 0x33;
                        l2McastAddr.mac.octet[2] = (ethctl->val >> 8) & 0xFF;
                        l2McastAddr.mac.octet[3] = ethctl->val & 0xFF;
                        l2McastAddr.mac.octet[4] = (ethctl->val2 >> 8) & 0xFF;
                        l2McastAddr.mac.octet[5] = ethctl->val2 & 0xFF;
                        rtk_add_snooping_entry(&l2McastAddr);
                ...
                case RTLDELSNOOP:
                    ...
                    rtk_del_snooping_entry(&l2McastAddr);
                    break;
                case RTLDUMPSNOOP:
                    rtk_dump_snooping_entry(0,0);
                    break;
		case RTLPORTISOLATIONGET:
		    rtk_port_isolation_get(UTP_PORT1, &portmask);
		    printk("UTP_PORT1\t%d\n", portmask.bits[0]);
		    rtk_port_isolation_get(UTP_PORT2, &portmask);
		    printk("UTP_PORT2\t%d\n", portmask.bits[0]);
		    rtk_port_isolation_get(UTP_PORT3, &portmask);
		    printk("UTP_PORT3\t%d\n", portmask.bits[0]);
		    rtk_port_isolation_get(UTP_PORT4, &portmask);
		    printk("UTP_PORT4\t%d\n", portmask.bits[0]);
		    break;
		case RTLPORTISOLATIONSET:
		    RTK_PORTMASK_CLEAR(portmask);
		    portmask.bits[0] = (unsigned int)ethctl->val;
		    RTK_PORTMASK_PORT_SET(portmask, EXT_PORT1);
		    rtk_port_isolation_set(ethctl->phy_addr, &portmask);
		    printk("set RTK port isolation finish\n");
		    break;
		case RTLPORTISOLATIONCLEAN:
		    RTK_PORTMASK_CLEAR(portmask);
		    RTK_PORTMASK_PORT_SET(portmask, UTP_PORT0);
		    RTK_PORTMASK_PORT_SET(portmask, UTP_PORT1);
		    RTK_PORTMASK_PORT_SET(portmask, UTP_PORT2);
		    ...
		    RTK_PORTMASK_PORT_SET(portmask, EXT_PORT1);
		    rtk_port_isolation_set(UTP_PORT1, &portmask);
		    rtk_port_isolation_set(UTP_PORT2, &portmask);
		    rtk_port_isolation_set(UTP_PORT3, &portmask);
		    rtk_port_isolation_set(UTP_PORT4, &portmask);
		    printk("set RTK port isolation clean\n");
		    break;
                case ETHWANCHANGE:
                    ephy_int_cnt++;
                    break;
...
#define ACTLED_ACTIVE   1
#define ACTLED_INACTIVE 0
static unsigned long rxCounterSum_old = 1;
static int gLanLinkLedState = 0, gLanActLedState = ACTLED_INACTIVE;
static unsigned short bpGpioLink = 0, bpGpioAct = 0;
struct timer_list gLanLinkLedTimer;
static void _lanAggregateLedTimerStart(void);
static void _lanAggregateLedTimerExpire(void);
extern void kerSysSetGpioState(unsigned short bpGpio, GPIO_STATE_t state);
extern int BpGetLanAggregateLinkGpio( unsigned short *pusValue );
extern int BpGetLanAggregateActGpio( unsigned short *pusValue );
/*!
 *  Timer for software to check/pull gpio about lan aggregate led.
 */
static void _lanAggregateLedTimerStart(void)
{
	init_timer(&gLanLinkLedTimer);
	gLanLinkLedTimer.function = (void*)_lanAggregateLedTimerExpire;
	gLanLinkLedTimer.expires = jiffies + (HZ / 50);
	add_timer(&gLanLinkLedTimer);
}
/*!
 *  Function for software to check/pull gpio about lan aggregate led.
 *
 *  @note   ethsw_get_hw_stats(port, port_stats) only work with wan port
 */
static void _lanAggregateLedTimerExpire(void)
{
    int port = 0;
    int linkPhyNum = 0;
    unsigned long rxCounterSum = 0, txCounterSum = 0;
    struct net_device_stats *port_stats = &global.net_device_stats_from_hw;

    for (port = 0; port < 5; port++)
    {
        memset(port_stats, 0, sizeof(*port_stats));
        bcmsw_get_hw_stats(port, 1/*extswitch*/, port_stats);
        rxCounterSum += (unsigned long)port_stats->rx_packets;
        txCounterSum += (unsigned long)port_stats->tx_packets;

        if (ETHSW_CORE->anonymous1[117] & (1 << port))  //SWITCH_CORE_LNKSTS - Link Status Summary Register
        {
            //check if any lan port is inserting
            ++linkPhyNum;
            //printk(KERN_NOTICE "Phy port %d link up.\n",port);
        }//else do nothing
    }

    // Make voltage difference to turn the led on
    if ((linkPhyNum > 0) && (gLanLinkLedState == 0))
    {
        // If there's lan port plug-in, turn the led on.
        kerSysSetGpioState(bpGpioLink, kGpioActive);
        kerSysSetGpioState(bpGpioAct, kGpioActive);
        gLanLinkLedState = 1; //led always on, not need to active gpio again
    }
    else if ((linkPhyNum > 0) && (gLanLinkLedState == 1))
    {
        // Blink the led when packets passing through.
        if (rxCounterSum != rxCounterSum_old)
        {
            if (gLanActLedState == ACTLED_ACTIVE)
            {
                kerSysSetGpioState(bpGpioAct, kGpioInactive);
                gLanActLedState = ACTLED_INACTIVE;
            }
            else
            {
                kerSysSetGpioState(bpGpioAct, kGpioActive);
                gLanActLedState = ACTLED_ACTIVE;
            }
        }
        else
        {
            // Packets number not changed.
            if (gLanActLedState == ACTLED_INACTIVE)
            {
                kerSysSetGpioState(bpGpioAct, kGpioActive);
                gLanActLedState = kGpioActive;
            }//else do nothing
        }
    }
    else if ((linkPhyNum == 0) && (gLanLinkLedState == 1))
    {
        // If all lan ports plug-out, turn the led off.
        kerSysSetGpioState(bpGpioLink, kGpioInactive);
        kerSysSetGpioState(bpGpioAct, kGpioInactive);
        gLanLinkLedState = 0;
    }

    rxCounterSum_old = rxCounterSum;
    _lanAggregateLedTimerStart();
}
#endif //defined(ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED)
...
#if defined(ZYXEL_SUPPORT_SW_LAN_AGGREGATE_LED)
    if ((BpGetLanAggregateLinkGpio(&bpGpioLink) | BpGetLanAggregateActGpio(&bpGpioAct)) != BP_SUCCESS)
    {
        printk(KERN_NOTICE "Get Lan Aggregate gpio fail !\n");
    }
    else
    {
        _lanAggregateLedTimerStart();
...
#ifdef SUPPORT_RTK_SWITCH//for RTL Switch
static inline int IsGmacPortRTLSwitch( int log_port )
{
    if ( gmac_info_pg->enabled && (log_port == enet_gmac_log_port_rtl_switch() ) )
        return 1;
    else
        return 0;
}
...
int enet_gmac_log_port_rtl_switch( void )
{
    ...
    logPort += MAX_SWITCH_PORTS; //external
    return logPort;
}
#endif
