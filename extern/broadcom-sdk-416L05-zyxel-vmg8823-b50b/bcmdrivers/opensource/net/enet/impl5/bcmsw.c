...
void extsw_rgmii_config(void)
{
	...
	/*Cj_Lai , For VMG9823/4825 extenal switch of LAN LED when linked with 10M or 100M*/
#if defined(ZYPRJ_VMG9823)
	data16 = 0x0024;
	extsw_wreg_wrap(PAGE_CONTROL, 0x12, &data16, sizeof(data16));
#endif
}
...
static void extsw_addl_wan_config(void)
{
    ...
    if ( g_addl_wan != -1 )
    {
        ...
        /* Add all dynamic learnt ARL on this port */
...
extern int ethsw_get_hw_switching_state(void);
#endif
...
void extsw_set_wanoe_portmap(uint16 wan_port_map)
{
	...
	if(!ethsw_get_hw_switching_state()){
		// do not thing here .... Let switching config reserve previous state
		/* 
		reg_value = PBMAP_ALL;
...
int bcmsw_disable_hw_switching(void)
{
    ...
    reg_value = PBMAP_ALL;
    if (g_addl_wan != -1) /* Don't disable learning on wan port */
    {
        reg_value |= GET_PORTMAP_FROM_LOGICAL_PORTMAP(g_addl_wan, 1);
    }
    extsw_wreg_wrap(PAGE_CONTROL, REG_DISABLE_LEARNING, &reg_value, 2);
...
void sf2_init_config_dbg(void)
{
    ...
    extsw_rreg_wrap(PAGE_CONTROL, REG_SWITCH_MODE, &val8, 1);
