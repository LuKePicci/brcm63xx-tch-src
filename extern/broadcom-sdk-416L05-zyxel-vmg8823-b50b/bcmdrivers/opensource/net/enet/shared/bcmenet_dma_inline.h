...
#define RTK_PORT_FROM_TAG(tag) (tag & 0x0f)
#endif
                                   ...
                                   uint32 *rxpktgood, uint32 *context_p, int *rxQueue)
{
    ...
    *rxQueue = global_channel;
...

	{
		...
		*phy_port_id = port_from_flag(dmaDesc.status);
#if defined(SUPPORT_RTK_SWITCH)
		if(*phy_port_id==4){
			((BcmEnet_hdr2*)(*pBuf))->brcm_type = 0x8100;
			*phy_port_id = RTK_PORT_FROM_TAG(((BcmEnet_hdr2*)(*pBuf))->brcm_tag);
			//printk("bcmeapi_rx_pkt==========phy_port_id11111111111 to loical port is %d,((BcmEnet_hdr2*)(*pBuf))->brcm_tag is %d\n",*phy_port_id,((BcmEnet_hdr2*)(*pBuf))->brcm_tag);//debug
			/*Offset 2 device*/
			if(*phy_port_id==1)
				*phy_port_id = 9;
			else if (*phy_port_id==2)
				*phy_port_id=10;
			else if(*phy_port_id==3)
				*phy_port_id = 11;
			else if (*phy_port_id ==4)
				*phy_port_id = 12;
			//printk("bcmeapi_rx_pkt==========phy_port_id222222222222222 to loical port is %d \n",*phy_port_id);//debug
		}else
			*phy_port_id = PHYSICAL_PORT_TO_LOGICAL_PORT(*phy_port_id, 0); /* Logical port for Internal switch port */
#else
		*phy_port_id = PHYSICAL_PORT_TO_LOGICAL_PORT(*phy_port_id, 0); /* Logical port for Internal switch port */
#endif	//SUPPORT_RTK_SWITCH
#endif
