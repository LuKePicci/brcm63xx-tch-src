...
#if defined(ZCFG_USED)	 /* VCAUTOHUNT, ZyXEL, John */
static UINT32 FindHuntServiceType(struct sk_buff *skb);
#endif

#ifdef ZYXEL_SW_INTERNET_LED_BLINK //__ZyXEL__, David, support SW Internet LED blinking.
int bcmxtmrt_dataforwarded(void);
extern int (*isDataTrafficForwarded)(void);
...
static int __init bcmxtmrt_init(void)
{
...
#if defined (CONFIG_BCM963138) || defined (CONFIG_BCM963381)
   ...
   **/
   kthread_bind(g_GlobalInfo.rx_thread, CPU0_ID);
#endif
                                                    ...
                                                    UINT32 uMark)
{
   ...
   else
   {
      ...
      else
      {
         ...
         UINT32 ptmPrioIdx = (uMark >> 3) & 0x1;
         int    port;

         /* Find the txdma channel configured with the subpriority */
         for (port = 0; port < MAX_PHY_PORTS; port++)
         {
            if ((txdma = pDevCtx->pTxPriorities[ptmPrioIdx][port][uMark & 0x7]))
               break;
         }
      }
   }
...
#endif
   {
	  ...
	  if(!pGi->vcAutoHuntWorking){
         ...
         default:
		  ...
		  matchedService = FindHuntServiceType(skb);
		  if(matchedService!=0){
...
static void MirrorPacket(char *mirrorIntf, pNBuff_t pNBuf)
{
   ...
   skb2->protocol = htons(ETH_P_802_3);
    ...
    /*+++ Kide@zyxel: insert stag for rtk switch */
    skb2->mac_header = skb2->data;
    if (strcmp(netDev->name,"eth3") == 0)
        skb2 = vlan_insert_tag(skb2, 1);
    else if (strcmp(netDev->name,"eth2") == 0)
        skb2 = vlan_insert_tag(skb2, 2);
    else if (strcmp(netDev->name,"eth1") == 0)
        skb2 = vlan_insert_tag(skb2, 3);
    else if (strcmp(netDev->name,"eth0") == 0)
        skb2 = vlan_insert_tag(skb2, 4);
    else {
        printk(CARDNAME ": unknown mirror dev=%s\n", netDev->name);
        return;
    }

    if (skb2 == NULL) {
        printk(CARDNAME ": vlan_insert_tag failure\n");
        return;
    }

    skb2->protocol = __constant_htons(ETH_P_8021Q);
#endif
...
int bcmxtmrt_dataforwarded(void)
{
   ...
   for (i = 0; i < MAX_DEV_CTXS; i++)
   {
	   ...
	   /*refer to bcmxtmrt_update_hw_stats()
	    *ifconfig tx packets counter, i.e. ptm0.3, atm0*/
	   if (pDevCtx != (PBCMXTMRT_DEV_CONTEXT) NULL)
	   {
		   txCounter += pDevCtx->DevStats.tx_packets;
	   }
   }
   if(lastTxCounter !=  txCounter)
   {
	   ret = 1;
   }
   ...
   static int lastTxHeaderIndex[XTM_TX_CHANNELS_MAX] = {0};
   ...
   for (i = 0; i < MAX_DEV_CTXS; i++)
   {
      ...
      if (pDevCtx != (PBCMXTMRT_DEV_CONTEXT) NULL)
      {
         for (j = 0; j < pDevCtx->ulTxQInfosSize; j++)
         {
            ...
            for (k = 0; k < XTM_TX_CHANNELS_MAX; k++ )
            {
               if (pGi->ulBlinkLed[k] == 0xff)
                  break;   // continue
               if (txdma->ulDmaIndex == pGi->ulBlinkLed[k])
               {
                  if (lastTxHeaderIndex[txdma->ulDmaIndex] != txdma->txHeadIndex)
                  {
                     lastTxHeaderIndex[txdma->ulDmaIndex] = txdma->txHeadIndex;
                     ret = 1;
                  }
                  break;
               }
   ...
   for( i = 0; i < XTM_TX_CHANNELS_MAX; i++ )
   {
      ...
      if (txdma->txEnabled) {
        //printk("txdma->ulDmaIndex %lu, old:%lu, new:%lu ", txdma->ulDmaIndex,(UINT32)lastTxHeaderIndex[txdma->ulDmaIndex], (UINT32)txdma->txHeadIndex);
        // printk("h %lu i %lu e %lu lh %lu h %lu\n", highesttxdma->ulDmaIndex, txdma->ulDmaIndex, (UINT32)txdma->txEnabled
        // 	,(UINT32)lastTxHeaderIndex[txdma->ulDmaIndex], (UINT32)txdma->txHeadIndex);
         for (j = 0; j < XTM_TX_CHANNELS_MAX; j++)
         {
            if (pGi->ulBlinkLed[j] == 0xff)
               break;   //continue;

            if (txdma->ulDmaIndex == pGi->ulBlinkLed[j])
            {
               if (lastTxHeaderIndex[txdma->ulDmaIndex] != txdma->txHeadIndex)
               {
                  lastTxHeaderIndex[txdma->ulDmaIndex] = txdma->txHeadIndex;
