...
/*+++ Kide@zyxel: hook function for rtk external switch */
#ifdef CONFIG_SUPPORT_RTK_SWITCH
void (*rtk_switch_cpu_port_learning_disable_hook)(bool disable) = NULL;
EXPORT_SYMBOL(rtk_switch_cpu_port_learning_disable_hook);
#endif
...
#ifdef CONFIG_SUPPORT_RTK_SWITCH
        /*+++ Kide@zyxel: need disable mac learning on the cpu port of the rtk switch to support mirroring */
        if (rtk_switch_cpu_port_learning_disable_hook) {
            if (pDevCtx->szMirrorIntfOut[0] != '\0' || pDevCtx->szMirrorIntfIn[0] != '\0')
                rtk_switch_cpu_port_learning_disable_hook(1);
            else
                rtk_switch_cpu_port_learning_disable_hook(0);
        }
#endif
      }
      break;
...
int bcmxtmrt_request(XTMRT_HANDLE hDev, UINT32 ulCommand, void *pParm)
{
   ...
   switch (ulCommand)
   {
      ...
      case XTMRT_CMD_STOP_ALL_TX_QUEUE:
         nRet = DoStopTxQueues(pDevCtx, (PXTMRT_TRANSMIT_QUEUE_ID)pParm);
         break;

      case XTMRT_CMD_START_ALL_TX_QUEUE:
         nRet = DoStartTxQueues(pDevCtx, (PXTMRT_TRANSMIT_QUEUE_ID)pParm);
         break;
...
#ifdef ZYXEL_SW_INTERNET_LED_BLINK //__ZyXEL__, David, support SW Internet LED blinking.
   isDataTrafficForwarded = bcmxtmrt_dataforwarded;
#endif
                        ...
                        PXTMRT_LINK_STATUS_CHANGE pLsc)
{
   ...
   UINT32 i;
#ifdef ZYXEL_SW_INTERNET_LED_BLINK //__ZyXEL__, David, support SW Internet LED blinking.
   PBCMXTMRT_DEV_CONTEXT pTmpDevCtx;
   int nNoDev = 1;
   int j;
#endif
   ...
   if (pLsc->ulLinkDataMask == 0)
   {
      ...
      blog_notify(UPDATE_NETDEVICE, pDevCtx->pDev, 0, 0);
      ...
      for (i = 0; i < pDevCtx->ulTxQInfosSize; i++)
      {
         if (pDevCtx->txdma[i])
         {
            ...
            for (j = 0; j < MAX_TRANSMIT_QUEUES; j++)
            {
               if (pDevCtx->txdma[i]->ulDmaIndex == pGi->ulBlinkLed[j])
               {
                  pGi->ulBlinkLed[j] = 0xff;
                  break;
               }
            }
#endif
...
//      pGi->ulNumTxBufsQdAll = 0;
   ...
   for (i = 0; i < MAX_DEV_CTXS; i++)
   {
      pTmpDevCtx = pGi->pDevCtxs[i];
      if ( pTmpDevCtx != (PBCMXTMRT_DEV_CONTEXT) NULL  && pDevCtx != pTmpDevCtx && pTmpDevCtx->ulOpenState == XTMRT_DEV_OPENED)
      {
         nNoDev = 0;
         break;
      }
   ...
   if (nNoDev && isDataTrafficForwarded == bcmxtmrt_dataforwarded) {
                          ...
                          PXTMRT_TRANSMIT_QUEUE_ID pTxQId)
{
   ...
   for (i = 0; i < pDevCtx->ulTxQInfosSize; i++)
   {
      ...
      if (txdma && pTxQId->ulQueueIndex == txdma->ulDmaIndex)
      {
         ...
         for (j=0; j < MAX_TRANSMIT_QUEUES; j++)
         {
            if (pGi->ulBlinkLed[j] == pTxQId->ulQueueIndex)
            {
               pGi->ulBlinkLed[j] = 0xff;
               break;
            }
